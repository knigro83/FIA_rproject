...
Looking at the seedling and adult sample sizes, we will exclude white fir, Rocky Mountain juniper, two needle pinyon, limber pine, and western redcedar in fire plots from the analysis, because these groups have very low numbers of plots with seedlings. Western larch will be excluded from insect/disease analysis. We also exclude singleleaf pinyon from the analysis because the plots in this analysis do not seem to capture the full range of this species (it's niche is weirdly cut off in climate space).  
...
env_vars_cut_alt <- env_vars %>%
  dplyr::select(-PLT_CN,-X,-RSCD,-INVYR,-ANN_INV,-CYCLE,-cmd,-mcmt,-mwmt,-ppt_sm,-DD5,-dd_0,-bffp,-effp,-emt,-tave_sm,-tave_wt,-eref)

env_vars_cut_alt %>%
  cor() %>%
  abs()>0.9

# pdf(file="C:/Users/katie/OneDrive - Colostate/OneDrive/FIA project/figures/pre_trans_pairs_alt_2023.pdf")
# 
# pairs(env_vars_cut_alt, lower.panel = NULL)
#  
# dev.off()
...
env_vars_cut_alt_trans <- env_vars_cut_alt %>%
  mutate(MSP_log = log(msp),SHM_log = log(shm), PPT_wt_log = log(ppt_wt), PPT_log=log(ppt), PAS_log = log(pas+0.999), AHM_log=log(ahm), VPDMAX_log=log(vpdmax), VPDMIN_log=log(vpdmin), .keep="unused")

env_vars_cut_alt_trans2 <- env_vars_cut_alt %>%
  dplyr::select(c(-vpdmin, -vpdmax,-td,-ahm,-shm,-pas,-tmean)) %>% 
  mutate(MSP_log = log(msp),PPT_wt_log = log(ppt_wt), PPT_log=log(ppt), .keep="unused")

# pdf(file="C:/Users/katie/OneDrive - Colostate/OneDrive/FIA project/figures/post_trans_pairs_alt_2023.pdf")
# 
# pairs(env_vars_cut_alt_trans, lower.panel = NULL)
#  
# dev.off()
# 
# pdf(file="C:/Users/katie/OneDrive - Colostate/OneDrive/FIA project/figures/post_trans_pairs_alt_2023_v2.pdf")
# 
# pairs(env_vars_cut_alt_trans2, lower.panel = NULL)
#  
# dev.off()
```
...
alt_clim_pca <- prcomp(env_vars_cut_alt_trans2, scale.=T)
summary(alt_clim_pca)
par(mfrow=c(1,1))
plot(alt_clim_pca,type="l")
autoplot(alt_clim_pca, loadings.colour="red", loadings=T, loadings.label=T, loadings.label.colour="red", scale=0)
alt_clim_pca$rotation
...
##varimax rotation
clim_pca_var <- varimax(clim_pca$rotation)
clim_pca_var
```
...
This plot shows the difference in seedling vs. adult positions on PC1 (seedling mean - adult mean), separated by species and disturbance type. Differences in means greater than zero indicate that seedlings occupy higher values on PC1 (cooler/wetter climates) than adults whereas negative differences in means indicate that seedlings occupy lower values on PC1 (warmer/drier climates) than adults. Asterisks indicate that seedling and adult positions on PC1 are significantly different (p\<0.05), as evaluated with a T-test.
...
Now for PC2, where postive differences mean that seedlings occupied higher values on PC2 (colder/drier climates) than adults, and negative differences mean that seedlings occupied lower values on PC2 (warmer/wetter climates).
...
Now I will use Bayesian methods to fit multivariate normal distributions to the distribution of points in PC space for each species x lifestage x disturbance combination. These take a while to run so I have saved them as objects and read them back in each time. 
...
```{r}
##This is just trying ppt and nffd instead of the PCs 

# iso_data_2d <- list()
# for(i in 1:length(adult_data_pcapts)){
#     temp_iso <- rbind(adult_data[[i]],seedling_data[[i]]) %>%
#   dplyr::select(c("nffd","ppt")) %>%
#   mutate(group=c(rep(1,nrow(adult_data[[i]])),rep(2,nrow(seedling_data[[i]]))),community=1) %>%
#   dplyr::rename(iso1=nffd,iso2=ppt)
#     iso_data_2d[[i]]<- temp_iso
# }
# names(iso_data_2d)<-names(adult_data)
# names(iso_data_2d)
# 
# siber.objs2d <- list()
# for(i in 1:length(iso_data_2d)){
#   temp.obj<- createSiberObject(as.data.frame(iso_data_2d[[i]]))
#   siber.objs2d[[i]]<- temp.obj
# }
# names(siber.objs2d)<-names(adult_data)
# 
...
# 
# ellipses.posterior.list2d <- list()
# for(i in 1:length(siber.objs2d)){
#   temp.mvn <- siberMVN(siber.objs2d[[i]], parms, priors)
#   ellipses.posterior.list2d[[i]] <- temp.mvn
#   }
# names(ellipses.posterior.list2d)<-names(siber.objs2d)
# 
# SEA.B.list2d <- list()
# for(i in 1:length(ellipses.posterior.list2d)){
#   temp.SEA.B <- siberEllipses(ellipses.posterior.list2d[[i]])
#   SEA.B.list2d[[i]] <- temp.SEA.B
# }
#  names(SEA.B.list2d)<-names(siber.objs2d)
#  
#  ###post bayes analysis
#  
# #look at niche margins
# 
# pos1_1 <- ellipses.posterior.list2d[[1]]$'1.1'
# 
# ellipse_margins_2d_list<- list()
# ellipse_margins_2d <- data.frame(xmax_adult=NA, xmin_adult=NA, ymax_adult=NA, ymin_adult=NA,
#                               xmax_seed=NA, xmin_seed=NA, ymax_seed=NA, ymin_seed=NA)
# 
# ellipmargfun<- function(ellipses.posterior.list2d_element){
# for(i in 1:nrow(ellipses.posterior.list2d_element$'1.1')){
# pos_adult_2d <- ellipses.posterior.list2d_element$'1.1'
# pos_seed_2d <- ellipses.posterior.list2d_element$'1.2'
# 
# ellipse_adult_2d<- addEllipse(mu = pos_adult_2d[i,5:6], sigma = matrix(pos_adult_2d[i,1:4],ncol=2,nrow=2), p.interval = 0.95)
# xmax_adult = max(ellipse_adult_2d[,1])
# ymax_adult = max(ellipse_adult_2d[,2])
# xmin_adult = min(ellipse_adult_2d[,1])
# ymin_adult = min(ellipse_adult_2d[,2])
# ellipse_seed_2d<- addEllipse(mu = pos_seed_2d[i,5:6], sigma = matrix(pos_seed_2d[i,1:4],ncol=2,nrow=2), p.interval = 0.95)
# xmax_seed = max(ellipse_seed_2d[,1])
# ymax_seed = max(ellipse_seed_2d[,2])
# xmin_seed = min(ellipse_seed_2d[,1])
# ymin_seed = min(ellipse_seed_2d[,2])
# ellipse_margins_2d[i,] = c(xmax_adult, xmin_adult, ymax_adult, ymin_adult,xmax_seed, xmin_seed, ymax_seed, ymin_seed)
# }
# return(ellipse_margins_2d)  
# }
# 
# 
# ellipmargfun(ellipses.posterior.list2d[[1]])
# 
#  ellipse_margins_2d_list<- lapply(X = ellipses.posterior.list2d, FUN=ellipmargfun)
# 
# ###now calculate with relativizing by adult niche width. 
# max1_rel_2d<- matrix(nrow=length(ellipse_margins_2d_list),ncol=4)
# min1_rel_2d<- matrix(nrow=length(ellipse_margins_2d_list),ncol=4)
# 
# max2_rel_2d<- matrix(nrow=length(ellipse_margins_2d_list),ncol=4)
# min2_rel_2d<- matrix(nrow=length(ellipse_margins_2d_list),ncol=4)
# 
# for(i in 1:length(ellipse_margins_2d_list)){
# max1diff_2d<- (ellipse_margins_2d_list[[i]][,'xmax_seed'] - ellipse_margins_2d_list[[i]][,'xmax_adult'])/ (ellipse_margins_2d_list[[i]][,'xmax_adult'] - ellipse_margins_2d_list[[i]][,'xmin_adult'])
# max1sum_2d<- c(names(ellipse_margins_2d_list)[i],mean(max1diff_2d),quantile(max1diff_2d, c(.025,.975)))
# max1_rel_2d[i,]<- max1sum_2d
# min1diff_2d<- (ellipse_margins_2d_list[[i]][,'xmin_seed'] - ellipse_margins_2d_list[[i]][,'xmin_adult'])/(ellipse_margins_2d_list[[i]][,'xmax_adult'] - ellipse_margins_2d_list[[i]][,'xmin_adult'])
# min1sum_2d<- c(names(ellipse_margins_2d_list)[i],mean(min1diff_2d),quantile(min1diff_2d, c(.025,.975)))
# min1_rel_2d[i,]<- min1sum_2d
# 
# max2diff_2d<- (ellipse_margins_2d_list[[i]][,'ymax_seed'] - ellipse_margins_2d_list[[i]][,'ymax_adult'])/(ellipse_margins_2d_list[[i]][,'ymax_adult'] - ellipse_margins_2d_list[[i]][,'ymin_adult'])
# max2sum_2d<- c(names(ellipse_margins_2d_list)[i],mean(max2diff_2d),quantile(max2diff_2d, c(.025,.975)))
# max2_rel_2d[i,]<- max2sum_2d
# min2diff_2d<- (ellipse_margins_2d_list[[i]][,'ymin_seed'] - ellipse_margins_2d_list[[i]][,'ymin_adult'])/(ellipse_margins_2d_list[[i]][,'ymax_adult'] - ellipse_margins_2d_list[[i]][,'ymin_adult'])
# min2sum_2d<- c(names(ellipse_margins_2d_list)[i],mean(min2diff_2d),quantile(min2diff_2d, c(.025,.975)))
# min2_rel_2d[i,]<- min2sum_2d
# }
# 
# ##need to create pc1means table
# 
# pc1.means<- vector()
# for(i in 1:length(adult_data_pcapts)){
...
# ################
# 
# max1_rel_2d_df <- as.data.frame(max1_rel_2d) %>% dplyr::rename(group=V1, mean=V2, lwr = V3, upr = V4) %>% mutate(mean=as.numeric(mean),lwr=as.numeric(lwr),upr=as.numeric(upr)) %>% 
...
#   filter(!species.name == "singleleaf pinyon")
# 
# min1_rel_2d_df <- as.data.frame(min1_rel_2d) %>% dplyr::rename(group=V1, mean=V2, lwr = V3, upr = V4) %>% mutate(mean=as.numeric(mean),lwr=as.numeric(lwr),upr=as.numeric(upr)) %>% 
#   mutate(sig = ifelse(lwr>0&upr>0, "yes", ifelse(lwr<0 & upr<0, "yes", "no"))) %>% 
#   mutate(species= substr(group, 0,3),
#          dist =  str_extract(group,"_.+_")) %>% 
#   mutate(dist= gsub("_","",dist),
...
#   filter(!species.name == "singleleaf pinyon")
# 
# max2_rel_2d_df <- as.data.frame(max2_rel_2d) %>% dplyr::rename(group=V1, mean=V2, lwr = V3, upr = V4) %>% mutate(mean=as.numeric(mean),lwr=as.numeric(lwr),upr=as.numeric(upr)) %>% 
#   mutate(sig = ifelse(lwr>0&upr>0, "yes", ifelse(lwr<0 & upr<0, "yes", "no"))) %>% 
#   mutate(species= substr(group, 0,3),
#          dist =  str_extract(group,"_.+_")) %>% 
#   mutate(dist= gsub("_","",dist),
...
#   filter(!species.name == "singleleaf pinyon")
# 
# min2_rel_2d_df <- as.data.frame(min2_rel_2d) %>% dplyr::rename(group=V1, mean=V2, lwr = V3, upr = V4) %>% mutate(mean=as.numeric(mean),lwr=as.numeric(lwr),upr=as.numeric(upr)) %>% 
#   mutate(sig = ifelse(lwr>0&upr>0, "yes", ifelse(lwr<0 & upr<0, "yes", "no"))) %>% 
#   mutate(species= substr(group, 0,3),
#          dist =  str_extract(group,"_.+_")) %>% 
#   mutate(dist= gsub("_","",dist),
...
# 
# 
# ggplot(min1_rel_2d_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
#   geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
#     geom_point(pch=21)+
#   geom_hline(yintercept=0)+
#   scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="nffd Mean")+
#   scale_color_manual(values=c("gray","black"))+
#   facet_wrap(~dist)+
#   ggtitle("nffd 2.5%")+
#   theme(axis.text.x = element_text(angle=50, hjust=1,size=8),axis.title.x = element_blank())
# 
# ggplot(max1_rel_2d_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
#   geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
#     geom_point(pch=21)+
#   geom_hline(yintercept=0)+
#   scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="nffd Mean")+
...
#   theme(axis.text.x = element_text(angle=50, hjust=1,size=8),axis.title.x = element_blank())
# 
# ggplot(min2_rel_2d_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
#   geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
#     geom_point(pch=21)+
#   geom_hline(yintercept=0)+
#   scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="nffd Mean")+
...
#   theme(axis.text.x = element_text(angle=50, hjust=1,size=8),axis.title.x = element_blank())
# 
# ggplot(max2_rel_2d_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
#   geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
#     geom_point(pch=21)+
#   geom_hline(yintercept=0)+
#   scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="nffd Mean")+
...
# 
# #relativized
# mu1_2d_rel<- matrix(nrow=length(ellipses.posterior.list2d),ncol=4)
# mu2_2d_rel<- matrix(nrow=length(ellipses.posterior.list2d),ncol=4)
# 
# for(i in 1:length(ellipses.posterior.list2d)){
# mu1_2d_reldiff<- (ellipses.posterior.list2d[[i]]$`1.2`[,'mu[1]'] - ellipses.posterior.list2d[[i]]$`1.1`[,'mu[1]'])/(ellipse_margins_2d_list[[i]][,'xmax_adult'] - ellipse_margins_2d_list[[i]][,'xmin_adult'])
# mu1_2d_relsum<- c(names(ellipses.posterior.list2d)[i],mean(mu1_2d_reldiff),quantile(mu1_2d_reldiff, c(.025,.975)))
# mu1_2d_rel[i,]<- mu1_2d_relsum
# 
# mu2_2d_reldiff<- (ellipses.posterior.list2d[[i]]$`1.2`[,'mu[2]'] - ellipses.posterior.list2d[[i]]$`1.1`[,'mu[2]'])/ (ellipse_margins_2d_list[[i]][,'ymax_adult'] - ellipse_margins_2d_list[[i]][,'ymin_adult'])
# mu2_2d_relsum<- c(names(ellipses.posterior.list2d)[i],mean(mu2_2d_reldiff),quantile(mu2_2d_reldiff, c(.025,.975)))
# mu2_2d_rel[i,]<- mu2_2d_relsum
# }
# mu1_2d_rel_df<- as.data.frame(mu1_2d_rel) %>% dplyr::rename(group=V1, mean=V2, lwr = V3, upr = V4) %>% mutate(mean=as.numeric(mean),lwr=as.numeric(lwr),upr=as.numeric(upr)) %>% 
#   mutate(sig = ifelse(lwr>0&upr>0, "yes", ifelse(lwr<0 & upr<0, "yes", "no"))) %>% 
#   mutate(species= substr(group, 0,3),
#          dist =  str_extract(group,"_.+_")) %>% 
#   mutate(dist= gsub("_","",dist),
...
#     filter(!species.name == "singleleaf pinyon")
# 
# mu2_2d_rel_df<- as.data.frame(mu2_2d_rel) %>% dplyr::rename(group=V1, mean=V2, lwr = V3, upr = V4) %>% mutate(mean=as.numeric(mean),lwr=as.numeric(lwr),upr=as.numeric(upr)) %>% 
#   mutate(sig = ifelse(lwr>0&upr>0, "yes", ifelse(lwr<0 & upr<0, "yes", "no"))) %>% 
#   mutate(species= substr(group, 0,3),
#          dist =  str_extract(group,"_.+_")) %>% 
#   mutate(dist= gsub("_","",dist),
...
# 
# ######relativized
# nffd_df_rel<- bind_rows(mu1_2d_rel_df %>% mutate(var="nffd centroid", species=as.numeric(species)),
# max1_rel_2d_df %>% mutate(var="nffd max"),
#  min1_rel_2d_df %>% mutate(var="nffd min")) %>% 
# #   mutate(mean=ifelse(var=="nffd max",mean+.8,ifelse(var=="nffd min", mean-.8,mean)),
# #          lwr=ifelse(var=="nffd max",lwr+.8,ifelse(var=="nffd min", lwr-.8,lwr)),
# #          upr=ifelse(var=="nffd max",upr+.8,ifelse(var=="nffd min", upr-.8,upr)))
#   left_join(pc1means.table)
# 
# nffd_reldiff<- ggplot(nffd_df_rel, aes(x=factor(number,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(number)), y=mean, fill=pc1speciesmean, col=sig))+
...
#   facet_grid(factor(var, levels=c("nffd max","nffd centroid","nffd min"), labels=c("97.5%","centroid","2.5%"))~dist, scales = "free")+
...
#   ggtitle("nffd")+
#   theme_bw()+
#   theme(axis.text.x = element_text(angle=60, hjust=1,size=9, color="black"),axis.title.x = element_text(size=12, color="black"),axis.text.y = element_text(size=10, colour = "black"),axis.title.y = element_text(size=12, color="black"), strip.text = element_text(size = 12), title=element_text(size=14), legend.position = "right")
# 
# 
# ppt_df_rel<- bind_rows(mu2_2d_rel_df %>% mutate(var="ppt centroid", species=as.numeric(species)),
# max2_rel_2d_df %>% mutate(var="ppt max"),
#  min2_rel_2d_df %>% mutate(var="ppt min")) %>% 
# #   mutate(mean=ifelse(var=="ppt max",mean+.8,ifelse(var=="ppt min", mean-.8,mean)),
# #          lwr=ifelse(var=="ppt max",lwr+.8,ifelse(var=="ppt min", lwr-.8,lwr)),
# #          upr=ifelse(var=="ppt max",upr+.8,ifelse(var=="ppt min", upr-.8,upr)))
#   left_join(pc1means.table)
# 
# ppt_reldiff<- ggplot(ppt_df_rel, aes(x=factor(number,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(number)), y=mean, fill=pc1speciesmean, col=sig))+
#   geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig),width=0.2,size=1)+
#   geom_point(pch=21, size=2)+
#   geom_hline(yintercept=0,lty=1)+
#   scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0.32942, name="nffd Mean")+
#   scale_color_manual(values=c("gray","black"),name="Significance")+
#   facet_grid(factor(var, levels=c("ppt max","ppt centroid","ppt min"), labels=c("97.5%","centroid","2.5%"))~dist, scales = "free")+
...
#   theme(axis.text.x = element_text(angle=60, hjust=1,size=9, color="black"),axis.title.x = element_text(size=12, color="black"),axis.text.y = element_text(size=10, colour = "black"),axis.title.y = element_text(size=12, color="black"), strip.text = element_text(size = 12), title=element_text(size=14), legend.position = "right")
# 
# ggarrange(nffd_reldiff, ppt_reldiff, nrow=2, common.legend = T, labels = c("A)","B)"), legend = "right")
#                                                                                   
```              

```{r}
# ##tying out just using nffd and ppt instead of the PCA
# iso_data_2d <- list()
# for(i in 1:length(adult_data_pcapts)){
#     temp_iso <- rbind(adult_data[[i]],seedling_data[[i]]) %>%
#   dplyr::select(c("nffd","ppt")) %>%
...
ggplot(ellipse_margins_diff_props_clean_long, aes(x=dist, y=value, col=species.name))+
  ...
  geom_hline(yintercept = 0)+
  scale_color_manual(values=c("#5cbb7a",
"#d760c5",
"#82b540",
"#924fb6",
"#d2a43d",
...
ggplot(ellipse_margins_diff_props_clean_long %>% filter(dist=="fire" & x=="1"), aes(x=species.name, y=value, col=pc1speciesmean))+
  facet_wrap(~factor(name,levels=c("x_low_prop","x_high_prop","y_low_prop","y_high_prop"), labels=c("PC1 2.5%", "PC1 97.5%", "PC2 2.5%","PC2 97.5%")))+
  geom_point()+
  geom_hline(yintercept = 0)+
  geom_hline(yintercept = c(0.5,0.75), col="gray", lty=2)+
  #scale_color_gradientn(colors=wes_palette("Zissou1",100,type = "continuous"))+
  scale_color_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=1, name="PC1 Mean")+
...
ggplot(ellipse_margins_diff_props_clean_long %>% filter(dist=="insect.disease" & x=="1"), aes(x=species.name, y=value, col=pc1speciesmean))+
  facet_wrap(~factor(name,levels=c("x_low_prop","x_high_prop","y_low_prop","y_high_prop"), labels=c("PC1 2.5%", "PC1 97.5%", "PC2 2.5%","PC2 97.5%")))+
  geom_point()+
  geom_hline(yintercept = 0)+
  geom_hline(yintercept = c(0.5,0.75), col="gray", lty=2)+
...
ggplot(min1_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
    geom_point(pch=21)+
  geom_hline(yintercept=0)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
  scale_color_manual(values=c("gray","black"))+
  facet_wrap(~dist)+
  ggtitle("PC1 2.5%")+
  theme(axis.text.x = element_text(angle=50, hjust=1,size=8),axis.title.x = element_blank())

ggplot(max1_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
    geom_point(pch=21)+
  geom_hline(yintercept=0)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
...
ggplot(min2_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
    geom_point(pch=21)+
  geom_hline(yintercept=0)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
...
ggplot(max2_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
    geom_point(pch=21)+
  geom_hline(yintercept=0)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
...
ggplot(max2_df, aes(x=dist, y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
    geom_point(pch=21)+
  geom_hline(yintercept=0)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
...
ggplot(min1_rel_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
    geom_point(pch=21)+
  geom_hline(yintercept=0)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
...
ggplot(max1_rel_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
    geom_point(pch=21)+
  geom_hline(yintercept=0)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
...
ggplot(min2_rel_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
    geom_point(pch=21)+
  geom_hline(yintercept=0)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
...
ggplot(max2_rel_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig))+
    geom_point(pch=21)+
  geom_hline(yintercept=0)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
...
pc1_df<- bind_rows(mu1_df %>% mutate(var="PC1 centroid", species=as.numeric(species)),
...
ggplot(pc1_df, aes(x=dist, y=mean, fill=pc1speciesmean, col=sig))+
...
ggplot(pc1_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig),width=0.2,size=1)+
...
pc2_df<- bind_rows(mu2_df %>% mutate(var="PC2 centroid", species=as.numeric(species)),
...
ggplot(pc2_df, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig),width=0.2,size=1)+
    geom_point(pch=21, size=3)+
  geom_hline(yintercept=c(-3,0,3),lty=1)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
...
pc1_reldiff<- ggplot(pc1_df_rel, aes(x=factor(number,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(number)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig),width=0.2,size=1)+
    geom_point(pch=21, size=2)+
  geom_hline(yintercept=0,lty=1)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
...
pc2_reldiff<- ggplot(pc2_df_rel, aes(x=factor(number,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(number)), y=mean, fill=pc1speciesmean, col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig),width=0.2,size=1)+
  geom_point(pch=21, size=2)+
  geom_hline(yintercept=0,lty=1)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0.32942, name="PC1 Mean")+
  scale_color_manual(values=c("gray","black"),name="Significance")+
  facet_grid(factor(var, levels=c("pc2 max","pc2 centroid","pc2 min"), labels=c("97.5%","centroid","2.5%"))~dist, scales = "free")+
  scale_y_continuous(breaks=seq(-.8,.8,.4))+
  #annotate(geom="text",x=4, y=-4.5, label="hot/dry",col="black", size=5 )+
  #annotate(geom="text", x=4, y=6,label="cold/wet",col="black", size=5)+
  ylab("Relativized Difference in Margin Position")+
...
png("new_fia_figure.png", width=4500, height=6000, res = 600)
...
ggplot(shifts_byspecies, aes(x=shift, fill=factor(species,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name))))+
  ...
  scale_fill_manual(values = c("#F37408","#EFAB1A","#ECBF1E","#EBC021","#DCC340","#B7B66D","#B4B46F","#B0B372","#ABB177","#A7B077","#A6B282","#7FA698","#73A3A0","#6DA2A1","#3A9AB2"), name="Species")+
  theme_bw()+
...
ggplot(overlap.table.long_cutoff,aes(x=number,y=mean,color=factor(species.name, levels=pc1means.table$species.name),alpha=diff,shape=diff))+
  scale_color_manual(values = c("#F37408","#EFAB1A","#ECBF1E","#EBC021","#DCC340","#B7B66D","#B4B46F","#B0B372","#ABB177","#A7B077","#A6B282","#7FA698","#73A3A0","#6DA2A1","#3A9AB2"), name="Species")+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4)+
  geom_errorbar(aes(ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1)+
  scale_alpha_manual(values=c(1,.7),labels=c("no","yes"),name="95% CI overlaps 0")+
  ...
  theme_bw()+
  theme(axis.text = element_text(size=24),legend.text = element_text(size=16),legend.title = element_text(size=24),legend.position = 'bottom',axis.title = element_text(size=24),plot.margin=margin(0,0.5,.6,.1,"cm"), axis.text.x=element_blank(), axis.title.x = element_blank(),strip.text = element_text(size=16,color="black"), panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank())

##see how many CIs are non-overlapping within disturbances

a<- overlap.table.long %>% 
  filter(disturbance == "fire" & type=="gained") %>% 
  filter(species.name=="Douglas-fir")

b<- overlap.table.long %>% 
  filter(disturbance == "fire" & type=="gained") %>% 
  filter(species.name=="western redcedar")

species.pairs <- t(combn(unique(overlap.table.long$species.name),2))
species.pairs[1,]

ci_overlap <- data.frame(comp = NA, eval=NA)

for(i in 1:nrow(species.pairs)){
  a <- overlap.table.long %>% filter(disturbance=="fire", type=="gained") %>%  filter(species.name == species.pairs[i,1])
  b <- overlap.table.long %>% filter(disturbance=="fire", type=="gained") %>%  filter(species.name == species.pairs[i,2])
  
if(a %>% pull(mean) > b %>% pull(mean)){ 
    eval <- a %>% pull(lwr) >  b %>% pull(upr)
    row <- data.frame(comp= paste(species.pairs[i,1],"_",species.pairs[i,2], sep=""), eval = eval)
...
n_seed_fire<- sample.append %>% dplyr::select(c(species.name.x,disturbance,n.seed)) %>% 
  filter(disturbance=="fire") %>% 
  arrange(n.seed)

n_seed_ID<- sample.append %>% dplyr::select(c(species.name.x,disturbance,n.seed)) %>% 
  filter(disturbance=="insect.disease") %>% 
  arrange(n.seed)

n_seed_none<- sample.append %>% dplyr::select(c(species.name.x,disturbance,n.seed)) %>% 
  filter(disturbance=="none") %>% 
  arrange(n.seed)

#fire
ggplot(overlap.table.long_cutoff %>% filter(disturbance=="fire"),aes(x=factor(species.name, levels=n_seed_fire$species.name),y=mean,alpha=diff,shape=diff))+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4)+
  geom_errorbar(aes(ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1)+
  scale_alpha_manual(values=c(1,.7),labels=c("no","yes"),name="95% CI overlaps 0")+
...
#insect/disease
ggplot(overlap.table.long_cutoff %>% filter(disturbance=="insect.disease"),aes(x=factor(species.name, levels=n_seed_ID$species.name),y=mean,alpha=diff,shape=diff))+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4)+
  geom_errorbar(aes(ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1)+
  scale_alpha_manual(values=c(1,.7),labels=c("no","yes"),name="95% CI overlaps 0")+
...
#none
ggplot(overlap.table.long_cutoff %>% filter(disturbance=="none"),aes(x=factor(species.name, levels=n_seed_none$species.name),y=mean,alpha=diff,shape=diff))+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4)+
  geom_errorbar(aes(ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1)+
  scale_alpha_manual(values=c(1,.7),labels=c("no","yes"),name="95% CI overlaps 0")+
...
#for all species
diff.plot_all_top<- ggplot(overlap.table %>% filter(species.name %in% pc1means.table$species.name[c(1,3,4,6:7,9,12:15)]),aes(x=disturbance,y=mean.net.expansion,color=disturbance, alpha=nodiff,shape=nodiff))+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4)+
  geom_errorbar(aes(ymin=lwr.net.expansion, ymax =upr.net.expansion), width=.5, position = position_dodge(0.9),size=1)+
  scale_color_manual(values=disturbance.colors, name="Disturbance",labels=c("fire","insect/disease","none"))+
  scale_alpha_manual(values=c(1,0.4),labels=c("no","yes"),name="95% CI overlaps 0")+
  scale_shape_manual(values=c(16,1),labels=c("no","yes"),name="95% CI overlaps 0")+
  facet_wrap(~factor(species.name, levels=c(pc1means.table$species.name[c(1,3,4,6:7,9,12:15)])), nrow=2, labeller = labeller(species.name = label_wrap_gen(20)))+
  ylab("Net expansion")+
  theme_bw()+
  theme(axis.text = element_text(size=24),legend.text = element_text(size=24),legend.title = element_text(size=24),legend.position = 'none',axis.title = element_text(size=24),plot.margin=margin(0,0.5,.6,.1,"cm"), axis.text.x=element_blank(), axis.ticks.x=element_blank(),axis.title.x = element_blank(),strip.text = element_text(size=16,color="black"))
...
diff.plot_all_bottom<- ggplot(overlap.table %>% filter(species.name %in% pc1means.table$species.name[c(2,5,8,10,11)]),aes(x=disturbance,y=mean.net.expansion,color=disturbance, alpha=nodiff,shape=nodiff))+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4)+
  geom_errorbar(aes(ymin=lwr.net.expansion, ymax =upr.net.expansion), width=.5, position = position_dodge(0.9),size=1)+
  scale_color_manual(values=disturbance.colors, name="Disturbance",labels=c("fire","insect/disease","none"))+
  scale_alpha_manual(values=c(1,0.4),labels=c("no","yes"),name="95% CI overlaps 0")+
  scale_shape_manual(values=c(16,1),labels=c("no","yes"),name="95% CI overlaps 0")+
  facet_wrap(~factor(species.name, levels=c(pc1means.table$species.name[c(2,5,8,10,11)])), nrow=1, labeller = labeller(species.name = label_wrap_gen(20)))+
  ylab("")+
...
diff.plot_all_top<- ggplot(overlap.table,aes(x=disturbance,y=mean.net.expansion,color=disturbance, alpha=nodiff,shape=nodiff))+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4)+
  geom_errorbar(aes(ymin=lwr.net.expansion, ymax =upr.net.expansion), width=.5, position = position_dodge(0.9),size=1)+
  scale_color_manual(values=disturbance.colors, name="Disturbance",labels=c("fire","insect/disease","none"))+
  scale_alpha_manual(values=c(1,0.4),labels=c("no","yes"),name="95% CI overlaps 0")+
  scale_shape_manual(values=c(16,1),labels=c("no","yes"),name="95% CI overlaps 0")+
  facet_wrap(~factor(species.name, levels=pc1means.table$species.name[c(1:7,9:15,8)]), nrow=3, labeller = labeller(species.name = label_wrap_gen(20)), scales="free_y")+
  ylab("Net growth")+
  theme_bw()+
  theme(axis.text = element_text(size=24),legend.text = element_text(size=24),legend.title = element_text(size=24),legend.position = 'bottom',axis.title = element_text(size=24),plot.margin=margin(0,0.5,.6,.1,"cm"), axis.text.x=element_blank(), axis.ticks.x=element_blank(),axis.title.x = element_blank(),strip.text = element_text(size=16,color="black"))

diff.plot_all_bottom<-ggplot(overlap.table %>% filter(!species.name=="western redcedar"),aes(x=disturbance,y=mean.net.expansion,color=disturbance, alpha=nodiff,shape=nodiff))+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4)+
  geom_errorbar(aes(ymin=lwr.net.expansion, ymax =upr.net.expansion), width=.5, position = position_dodge(0.9),size=1)+
  scale_color_manual(values=disturbance.colors, name="Disturbance",labels=c("fire","insect/disease","none"))+
  scale_alpha_manual(values=c(1,0.4),labels=c("no","yes"),name="95% CI overlaps 0")+
  scale_shape_manual(values=c(16,1),labels=c("no","yes"),name="95% CI overlaps 0")+
  facet_wrap(~factor(species.name, levels=pc1means.table$species.name[-8]), nrow=3, labeller = labeller(species.name = label_wrap_gen(20)))+
  ylab("Net growth")+
  theme_bw()+
  theme(axis.text = element_text(size=24),legend.text = element_text(size=24),legend.title = element_text(size=24),legend.position = 'bottom',axis.title = element_text(size=24),plot.margin=margin(0,0.5,.6,.1,"cm"), axis.text.x=element_blank(), axis.ticks.x=element_blank(),axis.title.x = element_blank(),strip.text = element_text(size=16,color="black"))

# png("figures/diffplot_top.png", width = 1100, height = 600)
# 
# diff.plot_all_top
# 
# dev.off()
# 
# 
# png("figures/diffplot_bottom.png", width = 1000, height = 600)
# 
# diff.plot_all_bottom
# 
# dev.off()
...
```{r}
#summarise by traits
overlap.table_traits_raw <- as.data.frame(matrix(unlist(overlap.list),nrow = 960000, ncol=5,byrow=FALSE))  
colnames(overlap.table_traits_raw)<-names(overlap.list[[1]])
overlap.table_traits<- overlap.table_traits %>% 
  mutate(names = names(overlap.list)) %>% 
  separate(names,into=c("species","disturbance","extra"),sep="_") %>% 
...
overlap.df.traits<- overlap.df %>% 
  ...
  dplyr::rename("species.name" = "species.name.x", "dist"="disturbance")) %>% 
  filter(!species.name=="singleleaf pinyon")
...
overlap.table_raw <- as.data.frame(matrix(unlist(overlap.list),nrow = 960000, ncol=5,byrow=FALSE))  
...
ggplot(distance_summary, aes(x=factor(species.name,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(species.name)), y=mean, fill=pc1speciesmean),col="black")+
...
ggplot(distance_summary, aes(x=dist, y=mean, fill=pc1speciesmean),col="black")+
  geom_errorbar(aes(ymin=lwr,ymax=upr), col="black")+
    geom_point(pch=21, size=3)+
  scale_fill_gradient2(low="#F21A00", mid = "#E9C925", high="#3A9AB2", midpoint=0, name="PC1 Mean")+
