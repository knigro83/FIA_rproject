...
for(i in seq(1,48,3)){
  png(file=paste("C:/Users/katie/OneDrive - Colostate/OneDrive/FIA project/FIA_rproject/ellipses/multi ellipses/justellipses_2024_",names(siber.objs)[i],".png", sep=""), res=120)
plot(x=unlist(c(siber.objs[[i+2]]$original.data %>% filter(group==1) %>% dplyr::select(iso1))),
     y=unlist(c(siber.objs[[i+2]]$original.data %>% filter(group==1)%>% dplyr::select(iso2))),col="white", xlim=c(min(siber.objs[[i+2]]$original.data %>% filter(group==1) %>% dplyr::select(iso1))-1,max(siber.objs[[i+2]]$original.data %>% filter(group==1) %>% dplyr::select(iso1))+1),ylim=c(min(siber.objs[[i+2]]$original.data %>% filter(group==1)%>% dplyr::select(iso2))-1,max(siber.objs[[i+2]]$original.data %>% filter(group==1)%>% dplyr::select(iso2))+1),pch=16, xlab="PC1",ylab="PC2", mgp=c(2,1,0),main=NA)
...
par(mfrow=c(1,1))
plot.new()
legend("bottomleft", c("Seedling-fire","Seedling-insect.disease","Seedling-none"), col = c(disturbance.colors), lty=2, lwd=2)
legend("bottomright", c("Adult-fire","Adult-insect.disease","Adult-none"), col = disturbance.colors, lty=1, lwd=2)
...
mu1_adult_df<-  as.data.frame(mu1_adult) %>% dplyr::rename(group=V1, mean_mu1=V2, lwr_mu1 = V3, upr_mu1 = V4) %>% mutate(mean_mu1=as.numeric(mean_mu1),lwr_mu1=as.numeric(lwr_mu1),upr_mu1=as.numeric(upr_mu1)) %>%
  mutate(species= substr(group, 0,3),
         dist =  str_extract(group,"_.+_")) %>%
  mutate(dist= gsub("_","",dist),
         species = as.character(gsub("_","",species))) %>%
  left_join(species.names,by=c("species"="species.code")) %>%
    left_join(pc1means.table)%>%
    filter(!species.name == "singleleaf pinyon") 
...
pc1_grpbydist<- ggplot(pc1_df_rel_new_cutxtreme, aes(x=number, y=mean, group=dist))+
  geom_rect(data = pc1_df_rel_new_cutxtreme, aes(xmin=number - 0.25, xmax=number + 0.25, ymin=-Inf, ymax=Inf, fill=PC1, group=dist),alpha=.5, position=position_dodge(0.8)) +
  ...
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig, group=dist),width=0.2,size=1, position=position_dodge(.8))+
  scale_fill_manual(values= c("#FDBF6F","#35978F","#74ADD1","#6A3D9A","white"), name="Shift Type")+
  ggnewscale::new_scale_fill()+
  geom_point(aes(fill=dist, col=sig), pch=21, size=3,position=position_dodge(.8))+
  ...
  #ggtitle("PC1")+
  theme_bw()+
  theme(axis.text.x = element_text(angle=45, hjust=1,size=9, color="black"),axis.title.x = element_blank(),axis.text.y = element_text(size=10, colour = "black"),axis.title.y = element_text(size=12, color="black"), strip.text = element_text(size = 10), title=element_text(size=12), legend.position = "right", legend.text = element_text(size=9), legend.title = element_text(size=10),panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
pc1_grpbydist2 <- ggplot(pc1_df_rel_new_cutxtreme, aes(x=number, y=mean, group=dist))+
  ...
  scale_fill_manual(values= c("#FDBF6F","#35978F","#74ADD1","#6A3D9A","white"), name="Shift Type")+
  ggnewscale::new_scale_fill()+
  geom_point(aes(fill=factor(dist_sig, levels=c("fire_no","insect.disease_no","none_no","fire_yes","insect.disease_yes","none_yes")), col=sig), pch=21, size=3,position=position_dodge(.8), show.legend = FALSE)+
  geom_hline(yintercept=0,lty="solid")+
...
pc2_grpbydist<- ggplot(pc2_df_rel_new_cutxtreme, aes(x=number, y=mean, group=dist))+
  geom_rect(data = pc2_df_rel_new_cutxtreme, aes(xmin=number - 0.25, xmax=number + 0.25, ymin=-Inf, ymax=Inf, fill=PC2, group=dist),alpha=.3, position=position_dodge(0.8)) +
...
fireplot <- ggplot()+
  ...
  geom_errorbar(data=fire_data_cutxtreme %>% filter(diff=="diff"), aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1.5, col="black")+
  geom_point(data=fire_data_cutxtreme %>% filter(diff=="diff"),aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),fill=factor(species.name, levels=pc1means.table$species.name,labels=pc1means.table$number),y=mean), shape=21, position = position_dodge(0.9),size=5, stroke=2, col="black")+
  geom_errorbar(data=fire_data_cutxtreme %>% filter(diff=="nodiff"), aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1.5, col="gray")+
  geom_point(data=fire_data_cutxtreme %>% filter(diff=="nodiff"),aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),y=mean), shape=21, position = position_dodge(0.9),size=5, stroke=2, fill="gray", col="gray")+
  ...
  geom_text(data=fire_data_cutxtreme %>% filter(star=="yes"), aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), y=0.6, label="*"),size=11)+
  geom_text(data=fire_data_cutxtreme %>% filter(species.name%in% c(star_table2 %>% filter(dist=="fire") %>% pull(species)) & type=="lost" & !species.name %in%insectonlyspp), aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), y=0.55, label="x"),size=6, col="black")+
  ylim(c(-.1,.7))+
  ylab("Proportion")+
  theme_bw()+
...
idplot <- ggplot()+
  geom_hline(yintercept=0, linetype=1, size=0.3)+
  geom_errorbar(data=id_data_cutxtreme %>% filter(diff=="diff"), aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1.5, col="black")+
  geom_point(data=id_data_cutxtreme %>% filter(diff=="diff"),aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),y=mean,fill = factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number)), position = position_dodge(0.9),size=5,stroke=2, shape=21, col="black")+
  scale_fill_manual(values = c(spp.colors$color[1:10],"gray",spp.colors$color[12:15]), name="Species")+
  geom_errorbar(data=id_data_cutxtreme %>% filter(diff=="nodiff"), aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1.5, col="gray")+
  geom_point(data=id_data_cutxtreme %>% filter(diff=="nodiff"),aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),y=mean),fill ="gray", position = position_dodge(0.9),size=5,stroke=2, shape=21, col="gray")+
  facet_grid(factor(type,levels=c("lost","gained"))~disturbance)+
  geom_text(data=id_data_cutxtreme %>% filter(star=="yes"), aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), y=0.6, label="*"),size=11)+
  geom_text(data=id_data_cutxtreme %>% filter(species.name%in% c(star_table2 %>% filter(dist=="insect.disease") %>% pull(species)) & type=="lost" & !species.name %in% fireonlyspp), aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), y=0.55, label="x"),size=6, col="black")+
  ylim(c(-.1,.7))+
  ylab("Proportion")+
  theme_bw()+
  theme(axis.text = element_text(size=24),legend.text = element_text(size=16),legend.title = element_text(size=24),legend.position = 'none',axis.title = element_text(size=24),plot.margin=margin(0,0.5,.6,.1,"cm"), axis.title.x = element_blank(),strip.text = element_text(size=20,color="black"), panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank(),axis.text.x = element_text(size=18, angle=60, color="black", hjust=1))
...
noneplot<-ggplot()+
  geom_hline(yintercept=0, linetype=1, size=0.3)+
    geom_errorbar(data=overlap.table.long_cutoff %>% filter(disturbance=="none"& diff=="diff"), aes(x = factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), ymin=lwr, ymax =upr), col="black", width=0, position = position_dodge(0.9),size=1.5)+
  geom_point(data=overlap.table.long_cutoff %>% filter(disturbance=="none"& diff=="diff"),col="black",aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),y=mean, fill=factor(species.name, levels=c(pc1means.table$species.name))), position = position_dodge(0.9),size=5,stroke=2, shape=21)+
  scale_fill_manual(values = spp.colors$color, labels=species.label, name="Species")+
  geom_errorbar(data=overlap.table.long_cutoff %>% filter(disturbance=="none"& diff=="nodiff"), aes(x = factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), ymin=lwr, ymax =upr), col="gray", width=0, position = position_dodge(0.9),size=1.5)+
  geom_point(data=overlap.table.long_cutoff %>% filter(disturbance=="none"& diff=="nodiff"),col="gray",aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),y=mean), fill="gray", position = position_dodge(0.9),size=5,stroke=2, shape=21)+
  facet_grid(factor(type,levels=c("lost","gained"))~disturbance)+
...
legendplot1<- ggplot(overlap.table.long_cutoff %>% filter(disturbance=="none"),aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),y=mean, fill=factor(diff, levels=c("diff","nodiff"), labels = c("yes","no")), color=factor(diff, levels=c("diff","nodiff"), labels = c("yes","no"))))+
  scale_fill_manual(values=c("black","gray"), name="Significance")+
  scale_color_manual(values=c("black","gray"), name="Significance")+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=3,stroke=2, shape=21)+
  geom_errorbar(aes(ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1)+
  facet_grid(factor(type,levels=c("lost","gained"))~disturbance)+
  ylim(c(-.1,.7))+
  ylab("Proportion")+
...
legendplot2 <- ggplot()+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
    geom_errorbar(data=overlap.table.long_cutoff %>% filter(disturbance=="none"), aes(x = factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), ymin=lwr, ymax =upr), col="black", width=0, position = position_dodge(0.9),size=1.5)+
  geom_point(data=overlap.table.long_cutoff %>% filter(disturbance=="none"),col="black",aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),y=mean, fill=factor(species.name, levels=c(pc1means.table$species.name))), position = position_dodge(0.9),size=4,stroke=1, shape=21)+
  scale_fill_manual(values = spp.colors$color, labels=species.label, name="Species")+
  facet_grid(factor(type,levels=c("lost","gained"))~disturbance)+
  geom_text(data=overlap.table.long_cutoff %>% filter(disturbance=="none") %>% filter(species.name%in% c(star_table2 %>% filter(dist=="none") %>% pull(species)) & type=="lost"), aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), y=0.55, label="x"),size=6, col="black")+
  ylim(c(-.1,.7))+
...
png("figures/gl_bypc1_legend2.png", width = 1200, height = 480)
ggarrange(fireplot, idplot, legendplot2, ncol=3)
dev.off()
...
arrowsplot_fire<- ggplot(cart_positions.all %>% filter(disturbance=="fire") %>% filter(!species.name=="western redcedar"), aes(x,y)) +
  ...
  #scale_color_manual(values=c("#d55f90","#5aa554","#ad74d6","#9ab13f","#6070d5","#cd9337","#563686","#45c097","#8b2965","#9b8a40","#ca71c0","#b95336","#628ed6","#ba4758"))+
  geom_text_repel(data=for_arrows.all %>% filter(!y==0) %>% filter(disturbance=="fire")%>% filter(!species.name=="western redcedar"),aes(label=species.number,x=x,y=y),size=3,hjust=-.4,vjust=.4,col="black")+
  annotate("text",x=0.15,y=0.48,label="cold/dry",size=3)+
  annotate(geom="text",x=1,y=-0.02,label="cold/wet",size=3)+
  annotate(geom="text",x=0,y=-0.18,label="hot/wet",size=3)+
  annotate(geom="text",x=-0.15,y= 0,label="hot/dry",size=3,angle=90)+
  ylab("PC2 (16.09%)") +
  xlab("PC1 (74.4%)") + 
  labs(title="Fire", size=12)+
  scale_y_continuous(limits=c(-.2,.5),expand=c(0,0))+
  scale_x_continuous(limits=c(-.2,1.2),expand=c(0,0))+
  theme(text = element_text(size=12,color="black"),legend.position = "none", plot.title = element_text(hjust = 0.5),
   axis.text.x = element_text(angle = 45, hjust = 1, vjust= 1, color="black"), axis.text.y = element_text(color="black"),panel.spacing.x = unit(25, "points"))
...
arrowsplot_insect.disease<- ggplot(cart_positions.all %>% filter(disturbance=="insect.disease") %>% filter(!species.name=="western larch"), aes(x,y)) +
  ...
  scale_color_manual(values=rep("grey50",15),labels=NULL,name=NULL)+
  #scale_color_manual(values=c("#d55f90","#5aa554","#ad74d6","#9ab13f","#6070d5","#cd9337","#563686","#45c097","#8b2965","#9b8a40","#ca71c0","#b95336","#628ed6","#ba4758"))+
  geom_text_repel(data=for_arrows.all %>% filter(!y==0) %>% filter(disturbance=="insect.disease")%>% filter(!species.name=="western larch"),aes(label=species.number,x=x,y=y),size=3,hjust=-.4,vjust=.4,col="black")+
  annotate("text",x=0.1,y=0.48,label="cold/dry",size=3)+
  annotate(geom="text",x=.4,y=-0.02,label="cold/wet",size=3)+
  annotate(geom="text",x=0,y=-0.18,label="hot/wet",size=3)+
  annotate(geom="text",x=-0.13,y= -0.02,label="hot/dry",size=3)+
  ylab("PC2 (16.09%)") +
  xlab("PC1 (74.4%)") + 
    labs(title="Insect/Disease",size=12)+
  scale_y_continuous(limits=c(-.2,.5),expand=c(0,0))+
...
arrowsplot_none<- ggplot(cart_positions.all %>% filter(disturbance=="none"), aes(x,y)) +
  ...
  scale_color_manual(values=rep("grey50",15),labels=NULL,name=NULL)+
  #scale_color_manual(values=c("#d55f90","#5aa554","#ad74d6","#9ab13f","#6070d5","#cd9337","#563686","#45c097","#8b2965","#9b8a40","#ca71c0","#b95336","#628ed6","#ba4758"))+
  geom_text_repel(data=for_arrows.all %>% filter(!y==0) %>% filter(disturbance=="none"),aes(label=species.number,x=x,y=y),size=3,hjust=-.4,vjust=.4,col="black")+
  annotate("text",x=0.1,y=0.48,label="cold/dry",size=3)+
  annotate(geom="text",x=.4,y=-0.02,label="cold/wet",size=3)+
  annotate(geom="text",x=0,y=-0.18,label="hot/wet",size=3)+
  annotate(geom="text",x=-0.13,y= -0.02,label="hot/dry",size=3)+
