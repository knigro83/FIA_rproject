...
ggplot(dd0_df_rel, aes(x=factor(number.y,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(number)), y=mean, fill=factor(number.y,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(number)), col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig),width=0.2,size=1)+
    geom_point(pch=21, size=2)+
  geom_hline(yintercept=0,lty=1)+
  scale_fill_manual(values=spp.colors$color, name="Species")+
...
ggplot(wintppt_df_rel, aes(x=factor(number,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(number)), y=mean, fill=factor(number,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(number)), col=sig))+
  geom_errorbar(aes(ymin=lwr,ymax=upr, col=sig),width=0.2,size=1)+
  geom_point(pch=21, size=2)+
  geom_hline(yintercept=0,lty=1)+
  scale_fill_manual(values=spp.colors$color, name="Species")+
...
ggplot(data=wintppt_df_rel_wremoval)+
  geom_errorbar(aes(x=factor(number,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(number)), ymin=lwr,ymax=upr, col=sig, group=dist),width=0.2,size=1,position=position_dodge(.8))+
  geom_point(aes(x=factor(number,levels=pc1means.table %>% arrange(pc1speciesmean) %>% pull(number)), y=mean, fill=dist, col=sig), pch=21, size=2, position=position_dodge(.8))+
  geom_hline(yintercept=0,lty=1)+
  scale_fill_manual(values=disturbance.colors, name="Disturbance")+
  scale_color_manual(values=c("lightgray","black"),name="Significance")+
  facet_wrap(~factor(var, levels=c("winter ppt max","winter ppt centroid","winter ppt min"), labels=c("upper","centroid","lower")), scales = "free", nrow=3)+
  scale_y_continuous(breaks=seq(-.8,.8,.4))+
  #annotate(geom="text",x=4, y=-4.5, label="hot/dry",col="black", size=5 )+
  #annotate(geom="text", x=4, y=6,label="cold/wet",col="black", size=5)+
  ylab("Relativized Difference in Margin Position")+
...
ggplot(data=dd0_df_rel_wremoval)+
    geom_rect(aes(xmin=number.y - 0.5, xmax=number.y + 0.5, ymin=-Inf, ymax=Inf, fill=n.sig>1),alpha=.5) +
  scale_fill_manual(values=c(NA,"orange"))+
      ggnewscale::new_scale_fill()+
  geom_errorbar(aes(x=number.y, ymin=lwr,ymax=upr, col=sig, group=dist),width=0.2,size=1,position=position_dodge(.8))+
  geom_point(aes(x=number.y, y=mean, fill=dist, col=sig), pch=21, size=2, position=position_dodge(.8))+
  geom_hline(yintercept=0,lty=1)+
  scale_fill_manual(values=disturbance.colors, name="Disturbance")+
  scale_color_manual(values=c("lightgray","black"),name="Significance")+
  facet_wrap(~factor(var, levels=c("dd0 max","dd0 centroid","dd0 min"), labels=c("97.5%","centroid","2.5%")), scales = "free",nrow=3)+
  #scale_y_continuous(breaks=seq(-5,5,1))+
  #annotate(geom="text",x=4, y=-4.5, label="hot/dry",col="black", size=5 )+
  #annotate(geom="text", x=4, y=6,label="cold/wet",col="black", size=5)+
  ylab("Relativized Difference in Margin Position")+
