...
pc1_grpbydist<- ggplot(pc1_df_rel_new_cutxtreme, aes(x=number, y=mean, group=dist))+
  geom_rect(data = pc1_df_rel_new_cutxtreme, aes(xmin=number - 0.25, xmax=number + 0.25, ymin=-Inf, ymax=Inf, fill=PC1, group=dist),alpha=.3, position=position_dodge(0.8)) +
...
png("figures/PC1_shifts_grpbydist.png", width=5000, height=3000, res = 600)
pc1_grpbydist
dev.off()
...
fire_data_cutxtreme <- fire_data %>% 
  ...
  bind_rows(data.frame(species.name=c("western redcedar","western redcedar"),number = c(8,8), disturbance=c("fire","fire"), mean=c(NA,NA), lwr=c(NA,NA), upr=c(NA,NA), type=c("gained","lost")))

fireplot <- ggplot(fire_data_cutxtreme,aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),color=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),y=mean,alpha=diff,shape=diff))+
  scale_color_manual(values=spp.colors$color, name="Species")+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4, stroke=2)+
  geom_errorbar(aes(ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1.5)+
...
id_data_cutxtreme <- id_data %>% 
  ...
  bind_rows(data.frame(species.name=c("western larch","western larch"),number = c(11,11), disturbance=c("insect.disease","insect.disease"), mean=c(NA,NA), lwr=c(NA,NA), upr=c(NA,NA), type=c("gained","lost")))

idplot <- ggplot(id_data_cutxtreme,aes(x=factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number),y=mean,col = factor(species.name, levels=pc1means.table$species.name, labels=pc1means.table$number), alpha=diff,shape=diff))+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4,stroke=2)+
  geom_errorbar(aes(ymin=lwr, ymax =upr), width=0, position = position_dodge(0.9),size=1.5)+
      scale_color_manual(values = c(spp.colors$color[1:10],"gray",spp.colors$color[12:15]), name="Species")+
