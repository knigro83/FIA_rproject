...
#for all species
diff.plot_all<- ggplot(overlap.table,aes(x=disturbance,y=mean.net.expansion,color=disturbance, alpha=nodiff))+
  geom_hline(yintercept=0, linetype=2, size=0.8)+
  geom_point(position = position_dodge(0.9),size=4)+
  geom_errorbar(aes(ymin=lwr.net.expansion, ymax =upr.net.expansion), width=.5, position = position_dodge(0.9),size=1)+
  scale_color_manual(values=disturbance.colors, name="Disturbance",labels=c("fire","insect/disease","none"))+
  scale_alpha_manual(values=c(1,0.5),labels=c("no","yes"),name="95% CI overlaps 0")+
...
```{r}
#combine plots
boxplots<- ggarrange(distance.boxplot,angle.boxplot,nrow=1,ncol=2, labels=c("B","C"), font.label = list(size=20))
...
exp.shade.mod3 <- lm(log(mean.exp.prop.adult)~shade.tolerance*disturbance+ shade.tolerance2,data=sample.append)
par(mfrow=c(2,2))
plot(exp.shade.mod3)
Anova(exp.shade.mod3, type='3')
summary(exp.shade.mod3)
...
pc1.exp.plot_new <- ggplot(data= sample.append)+
  ...
  annotate("text", label=("bold(PC1score~~italic(p) == 0.04)"),parse=TRUE, x=-0.7, y=-7,size=8,hjust=0)+
    ...
    #annotate("text", label=("PC1score~x~disturbance ~~italic(p) == 0.09"), x=-0.5, y=-6.9,parse=TRUE,  size=8,hjust=0)+
...
pc1.contr.plot_new<- ggplot(data=niche.shift.trait.table)+
  ...
  #ylim(c(-10,0))+
    ...
    annotate("text", label=("bold(PC1score~x~dist ~~italic(p) == 0.01)"), x=-0.8, y=-5.4,parse=TRUE,  size=8,hjust=0)+
  theme(axis.text = element_text(size=24),axis.title = element_text(size=24),legend.text = element_text(size=24),legend.title = element_text(size=24),plot.margin = unit(c(0.5, 0.5, 0, 0.5), "cm"))
