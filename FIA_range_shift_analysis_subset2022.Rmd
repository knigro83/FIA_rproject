...
#png("figures/FIA_pca22_subset.png", width = 700, height = 400)
...
diff.plot<- ggplot(overlap.table %>% filter(!species.name %in% c(insectonlyspp,fireonlyspp)),aes(x=species.name,y=diff,fill=disturbance))+
  ...
  ylab("Net Expansion")+
  annotate("text",x=8.5,y=0.15,label="expansion",size=7)+
  annotate("text",x=8.5,y=-0.25,label="contraction",size=7)+
  theme_bw()+
  theme(axis.text.x = element_text(angle=-25,vjust=1,hjust=1),axis.text = element_text(size=24),legend.text = element_text(size=24),legend.title = element_text(size=24),legend.position = 'none',axis.title = element_text(size=24),plot.margin=margin(0,0.5,.6,.1,"cm"))
...
arrowcentroidplot<- ggarrange(arrowsplot1,boxplots, nrow=1,ncol=2, widths=c(3,1))
arrowcentroidplot

pdf(file="figures/arrowsplot22_subset.pdf",width=15,height=8.75)
...
anova.letters.pc2<- data.frame(quant = rep(c("2.5","centroid","97.5"),3), agent=c(rep("fire",3),rep("insect.disease",3),rep("none",3)), letters=c("AB","AB","A","A","A","A","B","B","A"))
...
facet_wrap(~ factor(quant,levels=c("2.5","centroid","97.5")))+
  ...
  geom_text(data=anova.letters.pc1, aes(x=agent, y=-0.18,fill=NULL, label=letters), parse=TRUE, col="azure4", size=5)+
  ggtitle("Shifts on PC1")+
  ylab("Difference on PC1")+
  xlab("")+
  theme_bw()+
...
pc2nicheshiftsplot<- ggplot(percentile.results %>% filter(pc=="pc2") %>% filter(!quant %in% c("5","95")), aes(x=agent, y=estimate, color = agent, fill=agent))+
  ...
  geom_text(data=anova.letters.pc2, aes(x=agent, y=-0.1,fill=NULL, label=letters), parse=TRUE, col="azure4", size=5)+
  ggtitle("Shifts on PC2")+
  ylab("Difference on PC2")+
  xlab("")+
  theme_bw()+
...
#put them together
# png("percentile_shifts_figure.png", width= 600, height = 500)
# 
#  ggarrange(pc1nicheshiftsplot, pc2nicheshiftsplot, nrow=2, heights=c(0.87,1))
# 
# dev.off()
```
...
par(mfrow=c(2,2))
plot(dist.exp.anova)
summary(dist.exp.anova2)
Anova(dist.exp.anova2, type='3')

AICc(dist.exp.anova, dist.exp.anova2)

dist.contr.anova2<- lm(log(mean.contr.prop.adult)~factor(disturbance)*n.adult, data=sample.append)
plot(dist.contr.anova2)
summary(dist.contr.anova2)
Anova(dist.contr.anova2, type= '3')
...
par(mfrow=c(2,2))
plot(exp.shade.mod2)
Anova(exp.shade.mod2, type='3')
summary(exp.shade.mod2)
...
par(mfrow=c(2,2))
plot(exp.disp.mod2)
Anova(exp.disp.mod2, type="3")
summary(exp.disp.mod2)

AICc(exp.disp.mod,exp.disp.mod2) #both models are within 2 AICc
...
##sample size as effect
pc1.exp.mod2 <- lm(log(mean.exp.prop.adult)~ pc1speciesmean*disturbance + pc1speciesmean*n.adult, data=sample.append)
par(mfrow=c(2,2))
plot(pc1.exp.mod2)
Anova(pc1.exp.mod2, type="3")
summary(pc1.exp.mod2)
cor.test(sample.append$pc1speciesmean, sample.append$n.adult)
contrasts(factor(sample.append$disturbance))
...
##sample size as effect
contr.shade.mod2<- lm(log(mean.contr.prop.adult)~shade.tolerance*disturbance + shade.tolerance*n.adult,data=sample.append)
par(mfrow=c(2,2))
...
##sample size as effect
contr.disp.mod2<- lm(log(mean.contr.prop.adult)~dispersal*disturbance+dispersal*n.adult,data=sample.append)
par(mfrow=c(2,2))
plot(contr.disp.mod2)
Anova(contr.disp.mod2, type="3")
summary(contr.disp.mod2)
...
##sample size as effect
pc1.contr.mod2 <- lm(log(mean.contr.prop.adult)~ pc1speciesmean*disturbance+ pc1speciesmean*n.adult, data=sample.append)
par(mfrow=c(2,2))
plot(pc1.contr.mod2)
Anova(pc1.contr.mod2, type="3")
summary(pc1.contr.mod2)

AICc(pc1.contr.mod,pc1.contr.mod2) #first model stil has lower AIC, even though sample size is marginally significant. But the effect size of n.adult is super small so it's probably not that impactful
