...
loadings.df<- as.data.frame(clim_pca$rotation) %>% 
  mutate(x=c(-2.5,-2.5,-2.7,2,-2,2.5,2.7,2.5), y=c(-3,-1.5,0,-3.9,2.5,-2,1.3,2.4), names = c(str_wrap("mean temp coldest month", width=16),str_wrap("# frost free degree days",width=16), str_wrap("climate moisture  deficit", width=16),str_wrap("log(mean summer precip)",width=16), str_wrap( "log(summer heat moisture index)",width=16),str_wrap("log(winter precip)",width=16),str_wrap("log(precip  as snow)",width=16), str_wrap("log(degree days below 0)",width=16)))
...
autoplot(clim_pca,x=1,y=2,colour="grey",loadings.colour="red", loadings=T, loadings.label=F, loadings.label.colour="black", scale=0, loadings.label.vjust=c(.5,-.3,-.5,-.3,-.4,-.05,1.4,0), loadings.label.hjust=c(1.1,1,0,-.1,1.1,-.05,0,-.1), loadings.label.size=6, alpha=0.5)+
  ...
  annotate(geom="text", x=loadings.df$x,y=loadings.df$y, label=loadings.df$names) +
  theme_bw()+
