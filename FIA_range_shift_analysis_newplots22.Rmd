...
#for 8 species
diff.plot<- ggplot(overlap.table %>% filter(!(disturbance == 'fire' & species.name %in% insectonlyspp)) %>% filter(!(disturbance=="insect.disease" & species.name %in% fireonlyspp)),aes(x=species.name,y=diff,fill=disturbance))+
  geom_bar(stat='identity',position='dodge')+
  scale_fill_manual(values=disturbance.colors)+
