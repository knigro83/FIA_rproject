...
pc1.mod2.5 <- lm(pc1diff_2.5 ~ factor(disturbance), data=centroidshifts %>% filter(!species.name %in% insectonlyspp))
par(mfrow=c(2,2))
plot(pc1.mod2.5)
...
cond %>% 
  ...
  filter(CONDPROP_UNADJ < 1) %>% 
  group_by(PLT_CN) %>% 
  dplyr::summarise(fldyps = paste(list(unique(FLDTYPCD))))
