...
Now I will look at differences between disturbances in the amount of contraction/expansion. There are no significant differences between disturbance types and amount of contraction or expansion, however there is significantly more contraction across the board than expansion. This is true for both the set of species with all disturbances (n=8) and all species (n=14).
...
overlap.table.fullset <- overlap.table %>% 
  filter(!species.name %in% c("white fir","grand fir","limber pine","two needle pinyon","singleleaf pinyon","western redcedar"))
...
contr.weight.mod<- lm(log(mean.contr.prop.adult)~mean.weight.mg*factor(disturbance),data=niche.shift.trait.table %>% filter(!dispersal == "animal"))
par(mfrow=c(2,2))
...
ggarrange(shade.contr.plot, shade.exp.plot, weight.contr.plot, weight.exp.plot,pc1.contr.plot,pc1.exp.plot,  nrow=4, ncol=2,disp.contr.plot, disp.exp.plot, common.legend = TRUE)
```

Now I will do the trait analysis on the data set with all 14 species, including the 6 that only have data for undisturbed and insect/disease plots. 
