...
```{r}
##using SIBER to make niche ellipses
colnames(adult_data_pcapts[[1]])==colnames(seedling_data_pcapts[[1]]) #check that colnames are same

#combine seedling and adult sp=15, dist=fire
fire_15_iso <- rbind(adult_data_pcapts[[1]],seedling_data_pcapts[[1]]) %>% 
  dplyr::select(c("PC1","PC2")) %>% 
  mutate(group=c(rep(1,nrow(adult_data_pcapts[[1]])),rep(2,nrow(seedling_data_pcapts[[1]]))),community=1) %>% 
  dplyr::rename(iso1=PC1,iso2=PC2)

#create siber object
fire_15_siber <- createSiberObject(as.data.frame(fire_15_iso)
)
```
...
#loop for the rest
for(i in 37:length(ellipses.posterior.list)){
    cat(i, "\n")
temp.overlap <- bayesianOverlap("1.1","1.2",ellipses.posterior.list[[i]],draws=NULL,p.interval=0.95)
...
par(mfrow=c(1,1))
hist(overlap.list[[1]]$exp.prop.adult,breaks=50)
hist(overlap.list[[1]]$contr.prop.adult,breaks=50)
...
ggplot(overlap.table,aes(x=disturbance,y=mean.exp.prop.adult))+
  geom_boxplot()+
  geom_jitter(height=0,width=0.2, aes(color=species.name),pch=15,size=2,alpha=0.7)+
  scale_color_manual(values=c("#c9ce4e","#8b4cca","#6ccb5b","#c74a90","#8fcdb3","#d0503b","#708fc2","#c39553","#483262",
"#4a633f","#cd9bb5","#6c3631"))

ggplot(overlap.table,aes(x=disturbance,y=mean.contr.prop.adult))+
  geom_boxplot()
...
ggplot(overlap.table %>% 
  ...
  geom_boxplot()+
  scale_color_manual(c("red","orange"))+
  theme(axis.text.x = element_text(angle=45,vjust=0.9,hjust=0.8))
```

Interesting to see that fire and harvest seem to be the top 2 disuturbances driving expansion for most species. Let's test this with ANOVA.

```{r}
dist.exp.anova <- aov(log(mean.exp.prop.adult)~disturbance,data=niche.shift.trait.table)
plot(dist.exp.anova)
...
dist.contr.anova<- aov(log(mean.contr.prop.adult)~disturbance, data=niche.shift.trait.table)
...
exp.weight.mod<- lm(log(mean.exp.prop.adult)~scale(mean.weight.mg)*disturbance,data=niche.shift.trait.table %>% filter(! species.code %in% c(106,814,63)))
par(mfrow=c(2,2))
plot(exp.weight.mod)
...
#seed weight
#take out pinyon, gambel oak and juniper because they are primarily dispersed by animals
contr.weight.mod<- lm(mean.contr.prop.adult~scale(mean.weight.mg)*disturbance,data=niche.shift.trait.table %>% filter(! species.code %in% c(106,814,63)))
par(mfrow=c(2,2))
