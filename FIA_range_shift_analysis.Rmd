...
sample_seeds <- dist_seeds_joined %>% 
  filter(!is.na(Agent)) %>%
  filter(Rec.MEASYEAR > 2006) %>% #only look at seedling data from plots surveyed in the later chunk of surveys
  group_by(SPCD, Agent) %>% 
  dplyr::summarise(n_plots = n_distinct(PLT_CN))
datatable(sample_seeds, caption = "Seedling Sample Size")
```

Let's set the minimum number of plots at 100. There are 21 species in the FIA database that have 100 or more plots in one of the 3 disturbance types with adults present, but only 12 species with seedlings present in 30+ plots per disturbance type. The species codes for those 12 species are as follows:

```{r, echo=FALSE,warning=FALSE}
#first make a dataframe to translate species codes to names
species.names <- data.frame(species.code = c(15,17,19,63,93,101,106,108,113,122,202,242,746,814,803), species.name = c("white fir","grand fir","subalpine fir","alligator juniper","Engelmann spruce","two needle pinyon","lodgepole pine","ponderosa pine","Douglas-fir","western redcedar","trembling aspen","Gambel oak","limber pine","whitebark pine","western larch"))

#filter sample size table to species with adults in at least 100 plots 
sample_adults_over100_cmb <- adult_sample_cmbsurv %>% 
  filter(n_plots >= 100) %>% 
  filter(!Agent %in% c("other","harvest")) %>% 
  group_by(SPCD) %>% 
  dplyr::summarise(dist_over100 = n_distinct(Agent)) %>% 
...
#filter out species with too low sample size
...
datatable(sample_seeds_over100, caption="Seedlings in > 100 plots")
```
...
#saveRDS(clim_pca, file = "FIA_pca.RDS")

clim_pca <- readRDS("FIA_pca.RDS")
...
ggplot(pca_pts_by_dist[which(! pca_pts_by_dist$Agent %in% c("other","harvest")),], aes(x=Agent,y=PC1,fill=Agent))+
  geom_boxplot()+
  theme_bw()
...
ggplot(pca_pts_by_dist[which(! pca_pts_by_dist$Agent %in% c("other","harvest")),], aes(x=Agent,y=PC2,fill=Agent))+
  geom_boxplot()+
  theme_bw()
```
...
pc1.diffs.sep <- pc1.diffs.table %>% 
  ...
  mutate(species = as.numeric(species)) %>% 
  left_join(species.names,by=c("species" = "species.code")) %>% 
  filter(!(agent == "insect.disease" & species.name %in% c("ponderosa pine","Gambel oak")))
```
...
#need to do Wilcoxon Signed Rank Test for 5th percentile of none and 95th percentile of fire because data is not normal 
...
fire95.test <- pc1.diffs.sep %>% 
  filter(agent == "fire") %>% 
  pull("quant_95") %>% 
  wilcox.test(y = NULL, alternative = "two.sided", mu = 0, conf.int = TRUE, conf.level = 0.95)

fire95.p.df <- data.frame(agent = "fire", quant = "quant_95", p.value = fire95.test$p.value, estimate = fire95.test$estimate, lowerci = fire95.test$conf.int[1], upperci = fire95.test$conf.int[2], t=fire95.test$statistic, df=NA, wilcoxon="y")

##replace 5th percentile of harvest with wilcoxon test values
pvalues.pc1<- pvalues.pc1 %>% 
                rows_update(none5.p.df,by=c("agent","quant"))%>% 
                rows_update(fire95.p.df,by=c("agent","quant"))
...
pc1.diffs.fullset<- pc1.diffs.sep %>% 
  filter(!species.name %in% c("ponderosa pine","Gambel oak"))
...
data.names<- data.frame(name = names(adult_data_pcapts),num=seq(1,48,1))
...
sample_size$species<- as.character(sample_size$species)
shapiro.results.a %>% 
  filter(p<0.05) %>% 
...
ggplot(pc1.ttest.pvalues.sep, aes(x = agent, y = diff.means, fill = agent))+
  ...
  facet_wrap(~species.name,nrow=2)+
  ylim(c(-0.2,0.4))+
  ggtitle("PC1")+
  xlab("")+
...
multiplot(plotlist=pc2_qplotlist[17:24],cols=4)

##seedlings
multiplot(plotlist=pc2_splotlist,cols=4)
multiplot(plotlist=pc2_sqplotlist[1:12],cols=4)
multiplot(plotlist=pc2_sqplotlist[13:24],cols=4)
...
ggplot(comb_pcapts %>% filter(age=="adult"), aes(x=PC1, y=PC2, color=factor(species.name)))+
  #geom_point()+
    stat_ellipse(data=comb_pcapts %>% filter(age=="adult"),
                 aes(x=PC1, y=PC2, color=factor(species.name)),type="norm",level=0.75,cex=1.3)+
...
shade.tolerance <- c(4.33, 4.01,4.83, 2, 4.53, 1.44, 1.48,1.64,2.78,4.73, 1.21,2.09, 1.56, 1, 1.35)

#make drought tolerance dataframe
drought.tolerance <- c(1.91, 2.33,2.02,5,2.58,4.97, 4.21, 4.32, 2.62,2.23, 1.77, 4.97,4.72,5,2.42)
...
cor(trait.data[,3:5])
ggplot(trait.data,aes(x=drought.tolerance,y=shade.tolerance))+
  geom_point()+
  geom_smooth(method="lm")
cor.test(trait.data$drought.tolerance,trait.data$shade.tolerance)
#shade and drought tolerance are correlated so should I just pick one?
ggplot(trait.data,aes(x=drought.tolerance,y=mean.weight.mg))+
  geom_point()+
  geom_smooth(method="lm")
cor.test(trait.data$drought.tolerance,trait.data$mean.weight.mg)
#drought tolerance and mean weight are not significantly correlated.
```
...
#   }
# names(ellipses.posterior.list)<-names(adult_data_pcapts)
# 
# SEA.B.list <- list()
...
# }
# names(SEA.B.list)<-names(adult_data_pcapts)
...
color.list = rep(c("#F8766D","#7CAE00","#00BFC4","#C77CFF"),12)
...
addEllipse(siber.objs[[i]]$ML.mu[[1]][ , , 2],
                     ...
                     lty = 1,
                     lwd = 2)
legend(-6,-2,legend=c("Adult","Seedling"),col=c("black",alpha(color.list[i],0.6)), pch=16, lty=1, lwd=2,cex=1.2)
}
```
...
# } ##overlap list has 36 complete entries. Need to do the rest
# 
# names(overlap.list)<- names(ellipses.posterior.list)
# 
# save(overlap.list, file="overlap.list.RData")

```

```{r}
#read in overlap data
load("overlap.list.RData")
```
...
overlap.table <- as.data.frame(matrix(unlist(overlap.summary),nrow = 48, ncol=5,byrow=TRUE))  
...
Interesting to see that fire and harvest seem to be the top 2 disturbances driving expansion for most species. Let's test this with ANOVA.

```{r}
dist.exp.anova <- aov(log(mean.exp.prop.adult)~disturbance,data=overlap.table)
plot(dist.exp.anova)
summary(dist.exp.anova) #significant difference between disturbances in expansion
TukeyHSD(dist.exp.anova) #post-hoc comparisons shows that harvest is significantly higher than insect/disease and none, with fire in the middle. 
...
t.test(overlap.table$mean.exp.prop.adult, overlap.table$mean.contr.prop.adult)

```

###Trait Niche Analysis 
```{r}
...
exp.weight.mod<- lm(log(mean.exp.prop.adult)~mean.weight.mg*disturbance,data=niche.shift.trait.table %>% filter(! species.code %in% c(106,814,63)))
par(mfrow=c(2,2))
plot(exp.weight.mod)
Anova(exp.weight.mod, type='3')
summary(exp.weight.mod)

ggplot(data=niche.shift.trait.table %>% filter(! species.code %in% c(106,814,63)),aes(x=mean.weight.mg,y=mean.exp.prop.adult))+
  geom_point()+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  facet_wrap(~disturbance)
```
...
#shade tolerance
contr.shade.mod<- lm(mean.contr.prop.adult~shade.tolerance*disturbance,data=niche.shift.trait.table)
par(mfrow=c(2,2))
...
#seed weight
#take out pinyon, gambel oak and juniper because they are primarily dispersed by animals
contr.weight.mod<- lm(mean.contr.prop.adult~mean.weight.mg*disturbance,data=niche.shift.trait.table %>% filter(! species.code %in% c(106,814,63)))
par(mfrow=c(2,2))
...
contr.weight.mod.alt<- lm(mean.contr.prop.adult~disturbance+mean.weight.mg:disturbance-1,data=niche.shift.trait.table %>% filter(! species.code %in% c(106,814,63)))
...
weight.contr.plot <- ggplot(data=niche.shift.trait.table %>% filter(! species.code %in% c(106,814,63)),aes(x=mean.weight.mg,y=mean.contr.prop.adult,color=disturbance))+
  geom_point(size=3)+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  theme_bw()+
  xlab("Mean Seed Weight (mg)")+ ylab("Mean Contraction")+
...
weight.exp.plot <- ggplot(data=niche.shift.trait.table %>% filter(! species.code %in% c(106,814,63)),aes(x=mean.weight.mg,y=mean.exp.prop.adult,color=disturbance))+
  geom_point(size=3)+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  theme_bw()+
  xlab("Mean Seed Weight (mg)")+ ylab("Mean Expansion")+
  theme(axis.text = element_text(size=18),axis.title = element_text(size=18),legend.text = element_text(size=18),legend.title = element_text(size=18))

shade.contr.plot <- ggplot(data=niche.shift.trait.table,aes(x=shade.tolerance,y=mean.contr.prop.adult,color=disturbance))+
  geom_point(size=3)+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  theme_bw()+
  xlab("Shade Tolerance")+ ylab("Mean Contraction")+
  theme(axis.text = element_text(size=18),axis.title = element_text(size=18),legend.text = element_text(size=18),legend.title = element_text(size=18))

shade.exp.plot <- ggplot(data=niche.shift.trait.table,aes(x=shade.tolerance,y=mean.exp.prop.adult,color=disturbance))+
  geom_point(size=3)+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  theme_bw()+
  xlab("Shade Tolerance")+ ylab("Mean Expansion")+
  theme(axis.text = element_text(size=18),axis.title = element_text(size=18),legend.text = element_text(size=18),legend.title = element_text(size=18))

ggarrange(shade.contr.plot, shade.exp.plot, weight.contr.plot, weight.exp.plot, nrow=2, ncol=2, common.legend = TRUE)
```
...
pc1means.table <- as.data.frame(matrix(unlist(pc1.means),nrow=48,ncol=1))
...
ggplot(pc1means.table,aes(x=pc1mean,y=rep(1,48),color=species.code))+
  geom_point()

ggplot(pc1means.table,aes(x=species.code,y=pc1mean))+
  geom_boxplot()+
  coord_flip()
...
niche.shift.trait.table<-
  niche.shift.trait.table %>% 
  left_join(pc1means.table,by=c("species.code","disturbance","extra"))

pc1.exp.mod <- lm(log(mean.exp.prop.adult)~ pc1mean*disturbance, data=niche.shift.trait.table)
plot(pc1.exp.mod)
summary(pc1.exp.mod)

ggplot(niche.shift.trait.table, aes(x=pc1mean,y=mean.exp.prop.adult))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~disturbance)
...
ggplot(niche.shift.trait.table, aes(x=pc1mean,y=mean.contr.prop.adult))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~disturbance)
```
...
dist %>% 
  group_by(Agent) %>% 
  dplyr::summarise(mean.TSD = mean(Rec.MEASYEAR - DSTRBYR),
            min.TSD = min(Rec.MEASYEAR - DSTRBYR),
            max.TSD = max(Rec.MEASYEAR - DSTRBYR))

ggplot(dist, aes(x=Rec.MEASYEAR - DSTRBYR))+
  geom_histogram()+
  facet_wrap(~Agent)
