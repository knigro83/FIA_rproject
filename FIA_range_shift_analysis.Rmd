...
Looking at the seedling and adult sample sizes, we will exclude white fir, grand fir, limber pine, two needle pinyon, singleleaf pinyon, and western redcedar in fire plots from the analysis, because these groups have very low numbers of plots with seedlings. 
...
#FIA codes for all desired species (n=8) and disturbances (n=3)
...
autoplot(clim_pca,x=1,y=2,colour="grey",loadings.colour="black", loadings=T, loadings.label=T, loadings.label.colour="black", scale=0, loadings.label.vjust=-.5, loadings.label.size=8, alpha=0.5)+
  theme_bw()+
...
# pdf("C:/Users/Katie/Google Drive/FIA project/FIA_rproject/figures/FIA_pca.pdf", width = 10, height = 10)
# 
# autoplot(clim_pca,x=1,y=2,colour="grey",loadings.colour="black", loadings=T, loadings.label=T, loadings.label.colour="black", scale=0, loadings.label.vjust=-.5, loadings.label.size=8, alpha=0.5)+
#   theme_bw()+
#   theme(axis.text = element_text(color="black",size=18),text = element_text(size=18),legend.text = element_text(size=18))
...
diff.plot<- ggplot(overlap.table %>% filter(!(disturbance == 'fire' & species.name %in% c("limber pine","grand fir","two needle pinyon"))),aes(x=species.name,y=diff,fill=disturbance))+
  geom_bar(stat='identity',position='dodge')+
  scale_fill_manual(values=disturbance.colors)+
  theme(axis.text.x = element_text(angle=45,vjust=0.9,hjust=0.8),axis.text = element_text(size=24),legend.text = element_text(size=24),legend.title = element_text(size=24),axis.title = element_text(size=24))
...
ggplot(cart_positions.all, aes(x,y)) +
  geom_polygon(data=df.top.triangle, aes(x,y),fill=rect.colors[2],alpha=0.8)+
  geom_polygon(data=df.right.triangle, aes(x,y),fill=rect.colors[4],alpha=0.8)+
  geom_polygon(data=df.bottom.triangle, aes(x,y),fill=rect.colors[3],alpha=0.8)+
  geom_polygon(data=df.left.triangle, aes(x,y),fill=rect.colors[1],alpha=0.8)+
...
ggplot(median_vectors_all %>% filter(!species.name %in% c(insectonlyspp)), aes(x=disturbance, y=medDist, fill=disturbance))+
  geom_boxplot()+
  scale_fill_manual(values=disturbance.colors)+
  ylab("Median distance")+
  xlab("")+
...
ggplot(median_vectors_all, aes(x=disturbance, y=medAngle, fill=disturbance))+
  annotate("rect",xmin = -Inf, xmax = Inf, ymin=0.79, ymax = 2.35, fill=rect.colors[2], alpha = 0.7)+
  annotate("rect",xmin = -Inf, xmax = Inf, ymin=-0.79, ymax = 0.79, fill=rect.colors[4], alpha = 0.7)+
  annotate("rect",xmin = -Inf, xmax = Inf, ymin=-2.35, ymax = -0.79, fill=rect.colors[3], alpha = 0.7)+
  annotate("rect",xmin = -Inf, xmax = Inf, ymin=-3.14, ymax = -2.35, fill=rect.colors[1], alpha = 0.7)+
  annotate("rect",xmin = -Inf, xmax = Inf, ymin=2.35, ymax = 3.14, fill=rect.colors[1], alpha = 0.7)+
  geom_text(aes(x="none",y=2.745),nudge_x=0.45,label="hot/dry",size=5)+
  geom_text(aes(x="none",y=1.57),nudge_x=0.45,label="hot/wet",size=5)+
  geom_text(aes(x="none",y=0),nudge_x=0.45,label="cold/wet",size=5)+
  geom_text(aes(x="none",y=-1.57),nudge_x=0.45,label="cold/dry",size=5)+
  geom_text(aes(x="none",y=-2.745),nudge_x=0.45,label="hot/dry",size=5)+
  geom_boxplot()+
  ...
  geom_text(data= anova.stats.medang, aes(x=disturbance,y=y,label=letter),size=8)+
  theme_bw()+
  theme(text=element_text(size=26, color="black"),axis.text = element_text(color="black"),legend.position = "none")


##group raw results by disturbance
...
for(i in 1:length(ellipses.posterior.list)){
adult_pc1_quantiles<- quantile(mvrnorm(n=1000,mu=c(ellipses.posterior.list[[1]]$`1.1`[1,5:6]), Sigma = matrix(c(ellipses.posterior.list[[1]]$`1.1`[1,1:4]),nrow=2,ncol=2, byrow = FALSE))[1,],c(0.025,0.975))

adult_pc2_quantiles<- quantile(mvrnorm(n=1000,mu=c(ellipses.posterior.list[[1]]$`1.1`[1,5:6]), Sigma = matrix(c(ellipses.posterior.list[[1]]$`1.1`[1,1:4]),nrow=2,ncol=2, byrow = FALSE))[2,],c(0.025,0.975))

seedling_pc1_quantiles<- quantile(mvrnorm(n=1000,mu=c(ellipses.posterior.list[[1]]$`1.2`[1,5:6]), Sigma = matrix(c(ellipses.posterior.list[[1]]$`1.2`[1,1:4]),nrow=2,ncol=2, byrow = FALSE))[1,],c(0.025,0.975))

seedling_pc2_quantiles<- quantile(mvrnorm(n=1000,mu=c(ellipses.posterior.list[[1]]$`1.2`[1,5:6]), Sigma = matrix(c(ellipses.posterior.list[[1]]$`1.2`[1,1:4]),nrow=2,ncol=2, byrow = FALSE))[2,],c(0.025,0.975))

quantile.table[i,]<- c(adult_pc1_quantiles,adult_pc2_quantiles,seedling_pc1_quantiles,seedling_pc2_quantiles)
}
...
#pc1 97.5 ID 
#pc1 2.5 ID 
normal.dist.list<- c("none","insect.disease","insect.disease")
normal.quant.list<- c("pc2_2.5_diff","pc1_2.5_diff","pc1_97.5_diff")
...
normal.results.table<-data.frame(agent = NA, quant = NA, p.value = NA, estimate = NA, lowerci = NA, upperci = NA, t=NA, df=NA, wilcoxon=NA) 
for(i in 1:3){
normal.results.table<- rbind(normal.results.table,c(agent =  normal.dist.list[i], quant = normal.quant.list[i], p.value = normal.results[[i]]$p.value, estimate = normal.results[[i]]$estimate, lowerci = normal.results[[i]]$conf.int[1], upperci = normal.results[[i]]$conf.int[2], t=normal.results[[i]]$statistic, df=normal.results[[i]]$parameter, wilcoxon="n"))
}

percentile.results<- wilcox.results %>% 
  rows_update(normal.results.table,by=c("agent","quant")) %>% 
...
pvalues.percentileshifts<- percentile.results %>% 
  mutate(sym.pos = ifelse(p.value<0.06,0.17,NA))
##
...
ggplot(percentile.results %>% filter(pc=="pc1"), aes(x=agent, y=estimate, color = agent, fill=agent))+
  ...
  scale_color_manual(values = disturbance.colors)+
  scale_fill_manual(values = disturbance.colors)+
  geom_text(data = subset(pvalues.percentileshifts, !is.na(sym.pos)), aes(x=agent, y=sym.pos), label="*", color="black",size=8)+
  geom_hline(yintercept=0, color="black", lty=2, size=1)+
  ylim(c(-4,7.2))+
  coord_flip()+
facet_wrap(~ quant)+
...
ggarrange(shade.exp.plot,shade.contr.plot ,weight.exp.plot,  weight.contr.plot, pc1.exp.plot, pc1.contr.plot, nrow=4, ncol=2,disp.exp.plot,disp.contr.plot,  common.legend = TRUE, labels= c("a","","b","","c","","d"), font.label = list(size=24))

png("traitplot.png", width = 800, height = 1200)
