...
Let's set the minimum number of plots at 30. The table below shows that there are 20 species in the FIA database that have 30 or more plots in each disturbance type with adults present, but only 12 species with seedlings present in 30+ plots per disturbance type. The species codes are as follows:
...
table = list(cmd.q95.table.sep,cmd.q50.table.sep,cmd.q5.table.sep)
agent.list = unique(cmd.q95.table.sep$agent)
quant = c("quant_5","quant_50","quant_95")

wiltest(table[[1]],agent.list[1],"quant_95")

test.results<- list()
for(j in 1:length(agent.list)){
test.results[[j]] <-  wiltest(cmd.q95.table.sep,agent.list[j],"quant_95")
}
test.results[[4]]
```
...
I will first run a principal components analysis (PCA) on a suite of climate variables for all plots included in the analysis (n = 59,456). A larger list of climate variables was narrowed down to remove highly correlated variables (\>0.9). Some of the precipitation variables were log-transformed to make relationships linear, which is an assumption of PCA. The climate variable codes and their definitions are as follows:
...
Here we see that the first two components capture 88% of the variability in climate variables.
...
The variables most strongly correlated with PC1 are CMD, PAS, NFFD, DD_0, and MCMT. The variables most strongly correlated with PC2 are MSP and PPT_wt. So, PC1 seems to be a temperature related axis, while PC2 corresponds to precipitation.
...
These boxplots display the range of PC1 and PC2 coordinates that plots within each disturbance category occupy.
...
The first plot here shows shifts on the 5th, 50th and 95th percentiles for all species combined. The second plot is the result of a Wilcoxon Signed Rank test (non-parametric t-test), showing the estimate as a diamond and the 95% confidence interval as lines. Asterisks indicate whether the shift was significantly different from zero (p\<0.05).
...
This plot shows the difference in seedling vs. adult mean positions on PC1, separated by species and disturbance type. Asterisks indicate that the shift in mean was significantly different from zero (p\<0.05) as evaluated with a T-test.
...
#plot relationships
####shade tolerance
ggplot(pc1.diffs.traits, aes(x=shade.tolerance, y=abs(quant_5),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc1.diffs.traits, aes(x=shade.tolerance, y=abs(quant_50),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)
...
####drought tolerance
ggplot(pc1.diffs.traits, aes(x=drought.tolerance, y=abs(quant_5),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc1.diffs.traits, aes(x=drought.tolerance, y=abs(quant_50),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc1.diffs.traits, aes(x=drought.tolerance, y=abs(quant_95),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)
...
ggplot(pc1.diffs.traits[!pc1.diffs.traits$species.code %in% c(814,106),], aes(x=mean.weight.mg, y=abs(quant_5),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc1.diffs.traits[!pc1.diffs.traits$species.code %in% c(814,106),], aes(x=mean.weight.mg, y=abs(quant_50),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc1.diffs.traits[!pc1.diffs.traits$species.code %in% c(814,106),], aes(x=mean.weight.mg, y=abs(quant_95),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)
...
By running some linear models we see that shade tolerance is a marginally significant predictor of the magnitude of 50th percentile shifts on PC1 and a significant predictor of 95th percentile shifts on PC1, with both being negative relationships (species with higher shade tolerance shifted less). There are no significant relationships between drought tolerance or seed weight and shifts on PC1.
...
Now we'll do the same for shifts on PC2.

```{r}
#plot relationships
####shade tolerance
ggplot(pc2.diffs.traits, aes(x=shade.tolerance, y=abs(quant_5),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc2.diffs.traits, aes(x=shade.tolerance, y=abs(quant_50),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc2.diffs.traits, aes(x=shade.tolerance, y=abs(quant_95),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

####drought tolerance
ggplot(pc2.diffs.traits, aes(x=drought.tolerance, y=abs(quant_5),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc2.diffs.traits, aes(x=drought.tolerance, y=abs(quant_50),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc2.diffs.traits, aes(x=drought.tolerance, y=abs(quant_95),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)
...
####seed weight
##excluded gambel oak and pinyon as they were large outliers
ggplot(pc2.diffs.traits[!pc2.diffs.traits$species.code %in% c(814,106),], aes(x=mean.weight.mg, y=abs(quant_5),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc2.diffs.traits[!pc2.diffs.traits$species.code %in% c(814,106),], aes(x=mean.weight.mg, y=abs(quant_50),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)

ggplot(pc2.diffs.traits[!pc2.diffs.traits$species.code %in% c(814,106),], aes(x=mean.weight.mg, y=abs(quant_95),color=species.name))+
  geom_point()+
  stat_smooth(method = "lm", col="red")+
  facet_wrap(~agent)
```
