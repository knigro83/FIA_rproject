...
ggplot(cart_positions.all, aes(x,y,fill=disturbance)) + 
  geom_bin2d(bins = 20) +
  #scale_fill_gradient(list(c(low = "white", high = "orange"),c(low="white",high="blue"),c(low="white",high="red"))) +
  coord_cartesian(xlim = c(-.2, +.4), ylim = c(-.3, +.2)) +
  facet_wrap( ~ factor(species.name), scales = "fixed") + 
  theme_classic() +
  geom_path(data = for_arrows.all, 
            arrow = arrow(type = "open", length = unit(0.2, "cm")),
