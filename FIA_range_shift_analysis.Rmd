...
##this takes ~10 hours 
# overlap.list<-list()
# for(i in 10:length(ellipses.posterior.list)){
#     cat(i, "\n")
...
# } ##overlap list has 9 complete entries. Need to do the rest
# 
...
#seed weight
#take out pinyon, gambel oak and juniper because they are primarily dispersed by animals
contr.weight.mod<- lm(mean.contr.prop.adult~mean.weight.mg*disturbance,data=niche.shift.trait.table %>% filter(! species %in% c(106,814,63)))
par(mfrow=c(2,2))
...
weight.contr.plot <- ggplot(data=niche.shift.trait.table %>% filter(! species %in% c(106,814,63)),aes(x=mean.weight.mg,y=mean.contr.prop.adult,color=disturbance))+
  geom_point(size=3)+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  theme_bw()+
...
weight.exp.plot <- ggplot(data=niche.shift.trait.table %>% filter(! species %in% c(106,814,63)),aes(x=mean.weight.mg,y=mean.exp.prop.adult,color=disturbance))+
  geom_point(size=3)+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  theme_bw()+
