...
exp.weight.mod<- lm(log(mean.exp.prop.adult)~mean.weight.mg*disturbance,data=niche.shift.trait.table %>% filter(! species %in% c(106,814,63)))
par(mfrow=c(2,2))
plot(exp.weight.mod)
Anova(exp.weight.mod, type='3')
summary(exp.weight.mod)

ggplot(data=niche.shift.trait.table %>% filter(! species %in% c(106,814,63)),aes(x=mean.weight.mg,y=mean.exp.prop.adult))+
  geom_point()+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  facet_wrap(~disturbance)
```
