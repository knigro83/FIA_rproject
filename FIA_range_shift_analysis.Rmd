...
Let's set the minimum number of plots at 100. Since there are always less seedling plots than adult plots, we will look at the seedlings first. We will also only look at fire, insect/disease, and undisturbed categories since the other disturbance categories have very low sample sizes. 
...
There are 11 species in the FIA database that have 100 or more plots in at least 2 of the 3 disturbance types with seedlings present. 
...
Looking at the seedling and adult sample sizes, we will exclude grand fir, limber pine, and two needle pinyon in fire plots from the analysis, because these groups have very low numbers of plots with seedlings. Ponderosa pine in insect/disease plots is also below our threshold of 100, but we will include this species/disturbance group as the sample size (65) seems sufficiently large and previous exploratory analysis confirmed that this group was not an outlier in analyses.  
...
env_vars_cut_alt <- env_vars %>% 
  dplyr::select(-X1,-FIA_slope,-FIA_elev,-foldedaspect,-TD, -ppt, -tmean, -tmax, - tmin, -vpdmin, -vpdmax, -MWMT, -AHM, -DD5, -bFFP, -eFFP, -EMT, -Eref, -Tave_wt, -Tave_sm, -PPT_sm, -awc.total,-CMD, -MCMT)
...
autoplot(clim_pca,x=1,y=2,colour="grey",loadings.colour="black", loadings=T, loadings.label=T, loadings.label.colour="black", scale=0, loadings.label.vjust=-1.5, loadings.label.size=8, alpha=0.5)+
  theme_bw()+
...
```{r, echo=FALSE}
#color palette
disturbance.colors<- c("#CC6666", "#9999CC", "#66CC99")
...
All difference data is normal, so we can use t-tests for all quantiles and disturbances on PC1. 
...
#none 95th quantile -- NOT NORMAL
ggqqplot(none.diffs.pc2$quant_95)
shapiro.test(none.diffs.pc2$quant_95)
```

5th and 95th quantile shifts in undisturbed plots and 95th quantile shifts in insect/disease plots are non-normal
...
pvalues.pc2<- pvalues.pc2 %>% 
                ...
                rows_update(pc2.none95.p.df, by=c("agent","quant")) %>% 
...
ggplot(pc1.ttest.pvalues.sep, aes(x = agent, y = diff.means, fill = agent))+
  ...
  theme_bw()+
  theme(axis.text.x = element_blank(), axis.text=element_text(color="black", size=12), legend.text = element_text(color="black", size=12), text=element_text(color="black", size=12), strip.text = element_text(color="black", size=11), legend.position = c(1, 0),legend.justification = c(1, -0.3))
```
...
ggplot(pc2.ttest.pvalues.sep, aes(x = agent, y = diff.means, fill = agent))+
  ...
  labs(fill = "Disturbance Agent")+
  theme_bw()+
  theme(axis.text.x = element_blank(), axis.text=element_text(color="black",size=12), text=element_text(color="black",size=12), legend.text = element_text(color="black",size=12), strip.text = element_text(color="black",size=11),legend.position = c(1,0), legend.justification = c(1,-0.3))
```
...
plot(ellipses.posterior.list[[1]][[1]][,5],type="l")
...
disturbance.colors
color.list = c(disturbance.colors[2:3],rep(disturbance.colors,3),disturbance.colors[2:3],disturbance.colors,disturbance.colors[2:3],disturbance.colors[c(1,3)],rep(disturbance.colors,3))
...
overlap.list <- readRDS("overlap.list.update.nov.RDS")
names(overlap.list) <- names(ellipses.posterior.list)
...
overlap.table <- as.data.frame(matrix(unlist(overlap.summary),nrow = 33, ncol=5,byrow=TRUE))  
...
expansion.plot <- ggplot(overlap.table %>% filter(!(disturbance == "fire" & species.name %in% c("limber pine", "grand fir","two needle pinyon"))),aes(x=disturbance,y=mean.exp.prop.adult))+
  ...
  scale_color_manual(values=c("#c9ce4e","#8b4cca","#6ccb5b","#c74a90","#8fcdb3","#d0503b","#708fc2","#c39553","#483262",
"#4a633f","#cd9bb5","#6c3631"),name="Species")+
  xlab("")+
...
"#4a633f","#cd9bb5","#6c3631"),name="Species")+
  xlab("")+
  ylab("Proportion Contracted")+   ylim(c(-0.001,0.35))+
  theme_bw()+
...
Now I will look at differences between disturbances in the amount of contraction/expansion. There are no significant differences between disturbance types and amount of contraction or expansion, however there is significantly more contraction across the board than expansion. 

```{r, echo=FALSE}
overlap.table.partialset <- overlap.table %>% 
  filter(!(disturbance == "fire" & species.name %in% c("grand fir","limber pine","two needle pinyon"))) 

overlap.table.fullset <- overlap.table %>% 
  filter(!species.name %in% c("grand fir","limber pine","two needle pinyon"))
...
hist(overlap.table.fullset$mean.contr.prop.adult)
...
trait.data <- data.frame(species.name = species.names$species.name, shade.tolerance=c(4.01,4.83,4.53,1,1.44,1.48,1.56,1.64,2.78,1.21,2.09))
...
#shade tolerance
exp.shade.mod<- lm(log(mean.exp.prop.adult)~shade.tolerance*disturbance,data=niche.shift.trait.table %>% filter(!species.name %in% c("grand fir","two needle pinyon","limber pine")))
par(mfrow=c(2,2))
plot(exp.shade.mod)
Anova(exp.shade.mod, type='3')
summary(exp.shade.mod)

ggplot(niche.shift.trait.table %>% filter(!species.name %in% c("grand fir","two needle pinyon","limber pine","ponderosa pine")),aes(x=shade.tolerance,y=mean.exp.prop.adult))+
  geom_point()+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  facet_wrap(~disturbance)

#seed weight
#take out pinyon, gambel oak and juniper because they are primarily dispersed by animals
exp.weight.mod<- lm(log(mean.exp.prop.adult)~mean.weight.mg*disturbance,data=niche.shift.trait.table %>% filter(!species.name %in% c("grand fir","two needle pinyon","limber pine","Gambel oak")))
par(mfrow=c(2,2))
plot(exp.weight.mod)
Anova(exp.weight.mod, type='3')
summary(exp.weight.mod)

ggplot(data=niche.shift.trait.table %>% filter(!species.name %in% c("grand fir","two needle pinyon","limber pine","ponderosa pine","Gambel oak")),aes(x=mean.weight.mg,y=mean.exp.prop.adult))+
  geom_point()+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  facet_wrap(~disturbance)
```
...
#shade tolerance
contr.shade.mod<- lm(log(mean.contr.prop.adult)~shade.tolerance*disturbance,data=niche.shift.trait.table %>% filter(!species.name %in% c("grand fir","two needle pinyon","limber pine")))
par(mfrow=c(2,2))
plot(contr.shade.mod)
Anova(contr.shade.mod, type='3')
summary(contr.shade.mod)

ggplot(niche.shift.trait.table %>% filter(!species.name %in% c("grand fir","two needle pinyon","limber pine")),aes(x=shade.tolerance,y=mean.contr.prop.adult))+
  geom_point()+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  facet_wrap(~disturbance)
...
#seed weight
#take out pinyon, gambel oak and juniper because they are primarily dispersed by animals
contr.weight.mod<- lm(log(mean.contr.prop.adult)~mean.weight.mg*factor(disturbance),data=niche.shift.trait.table %>% filter(! species %in% c(106,814,63)))
par(mfrow=c(2,2))
...
contr.weight.mod.alt<- lm(mean.contr.prop.adult~disturbance+mean.weight.mg:disturbance-1,data=niche.shift.trait.table %>% filter(! species %in% c(106,814,63)))
summary(contr.weight.mod.alt)

#these don't work right now, wrong # of disturbances ##
FslopevHslope <- c(0, 0, 0, 0, -1, 1, 0, 0)
lht(contr.weight.mod.alt, FslopevHslope, rhs = c(0))
FslopevIDslope <- c(0, 0, 0, 0, -1, 0, 1, 0)
lht(contr.weight.mod.alt, FslopevIDslope, rhs = c(0))
FslopevNslope <- c(0, 0, 0, 0, -1, 0, 0, 1)
lht(contr.weight.mod.alt, FslopevNslope, rhs = c(0))
HslopevIDslope <- c(0, 0, 0, 0, 0, -1, 1, 0)
lht(contr.weight.mod.alt, HslopevIDslope, rhs = c(0))
HslopevNslope <- c(0, 0, 0, 0, 0, -1, 0, 1)
lht(contr.weight.mod.alt, HslopevNslope, rhs = c(0))
IDslopevNslope <- c(0, 0, 0, 0, 0, 0, -1, 1)
lht(contr.weight.mod.alt, IDslopevNslope, rhs = c(0))

weight.contr.plot <- ggplot(data=niche.shift.trait.table %>% filter(! species %in% c(106,814,63)),aes(x=mean.weight.mg,y=log(mean.contr.prop.adult),color=disturbance))+
  geom_point(size=1)+
...
```{r}
#other 3 plots for insignificant models
weight.exp.plot <- ggplot(data=niche.shift.trait.table %>% filter(! species %in% c(106,814,63)),aes(x=mean.weight.mg,y=log(mean.exp.prop.adult),color=disturbance))+
  geom_point(size=1)+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=.5)+
  theme_bw()+
...
ggarrange(shade.contr.plot, shade.exp.plot, weight.contr.plot, weight.exp.plot,pc1.contr.plot,pc1.exp.plot,  nrow=3, ncol=2, common.legend = TRUE)
```
...
```{r}
#colors
species.colors <- c("#c9ce4e","#8b4cca","#6ccb5b","#c74a90","#8fcdb3","#d0503b","#708fc2","#c39553","#4a633f","#cd9bb5","#6c3631")

"#483262"
#plot all species together
ggplot(comb_pcapts %>% filter(age=="adult"), aes(x=PC1, y=PC2, color=factor(species.name)))+
  #geom_point()+
    stat_ellipse(data=comb_pcapts %>% filter(age=="adult"),
...
ggplot(comb_pcapts %>% filter(age=="adult"), aes(x=PC1, y=PC2, color=factor(species.name)))+
  #geom_point()+
    stat_ellipse(data=comb_pcapts %>% filter(age=="adult"),
                 aes(x=PC1, y=PC2, color=factor(species.name, levels=c("two needle pinyon","Gambel oak","ponderosa pine","Douglas-fir","limber pine","trembling aspen","grand fir","lodgepole pine","Engelmann spruce","subalpine fir","whitebark pine")),alpha=factor(species.name, levels=c("two needle pinyon","Gambel oak","ponderosa pine","Douglas-fir","limber pine","trembling aspen","grand fir","lodgepole pine","Engelmann spruce","subalpine fir","whitebark pine"))),type="norm",level=0.5,cex=1.3)+
  scale_color_manual(values=c("#c9ce4e","#8b4cca","#6ccb5b","#c74a90","#8fcdb3","#d0503b","#708fc2","#c39553","#483262",
"#4a633f","#cd9bb5","#6c3631"),name="Species")+
  scale_alpha_manual(values=c(1,1,1,1,1,1,1,rep(1,3),1),guide='none')+
  theme_bw()+
  theme(axis.text = element_text(size=16), axis.title = element_text(size=16), legend.title = element_text(size=16),legend.text = element_text(size=16))
