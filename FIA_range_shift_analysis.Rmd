...
Difference data on the 5th quantile of PC1 for harvested plots is not normal. 
...
#need to do Wilcoxon Signed Rank Test for 5th percentile of harvest because data is not normal 
harvest5.test <- pc1.diffs.sep %>% 
  filter(agent == "harvest") %>% 
  pull("quant_5") %>% 
  wilcox.test(y = NULL, alternative = "two.sided", mu = 0, conf.int = TRUE, conf.level = 0.95)

harvest5.p.df <- data.frame(agent = "harvest", quant = "quant_5", p.value = harvest5.test$p.value, estimate = harvest5.test$estimate, lowerci = harvest5.test$conf.int[1], upperci = harvest5.test$conf.int[2], t=harvest5.test$statistic, df=NA, wilcoxon="y")
...
pc2.fire50.p.df <- data.frame(agent = "fire", quant = "quant_50", p.value = pc2.fire50.test$p.value, estimate = pc2.fire50.test$estimate, lowerci = pc2.fire50.test$conf.int[1], upperci = pc2.fire50.test$conf.int[2],t=pc2.fire50.test$statistic,df=NA,wilcoxon="y")
...
pc2.harvest5.p.df <- data.frame(agent = "harvest", quant = "quant_5", p.value = pc2.harvest5.test$p.value, estimate = pc2.harvest5.test$estimate, lowerci = pc2.harvest5.test$conf.int[1], upperci = pc2.harvest5.test$conf.int[2],t=pc2.harvest5.test$statistic,df=NA,wilcoxon="y")
...
pc2.harvest95.p.df <- data.frame(agent = "harvest", quant = "quant_95", p.value = pc2.harvest95.test$p.value, estimate = pc2.harvest95.test$estimate, lowerci = pc2.harvest95.test$conf.int[1], upperci = pc2.harvest95.test$conf.int[2],t=pc2.harvest95.test$statistic,df=NA,wilcoxon="y")

#5th id
pc2.id5.test <- pc2.diffs.sep %>% 
  filter(agent == "insect.disease") %>% 
  pull("quant_5") %>% 
  wilcox.test(y = NULL, alternative = "two.sided", mu = 0, conf.int = TRUE, conf.level = 0.95)

pc2.id5.p.df <- data.frame(agent = "insect.disease", quant = "quant_5", p.value = pc2.id5.test$p.value, estimate = pc2.id5.test$estimate, lowerci = pc2.id5.test$conf.int[1], upperci = pc2.id5.test$conf.int[2],t=pc2.id5.test$statistic,df=NA,wilcoxon="y")
...
pvalues.pc2<- pvalues.pc2 %>% 
                ...
                rows_update(pc2.id5.p.df, by=c("agent","quant"))
pvalues.pc2
```
...
ggplot(data=niche.shift.trait.table %>% filter(! species.code %in% c(106,814,63)),aes(x=mean.weight.mg,y=mean.contr.prop.adult))+
  geom_point()+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  facet_wrap(~disturbance)
```
