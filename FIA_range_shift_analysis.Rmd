...
prev_plot_spp <- plot_progression %>% 
  ...
  full_join(dist_adults_joined, by="PLT_CN")

length(unique(prev_plot_spp$PREV_PLT_CN))
length(unique(prev_plot_spp$PLT_CN))
length(unique(plot_progression$PLT_CN))
length(unique(plot_progression$PREV_PLT_CN))
length(unique(dist_adults_joined$PLT_CN))

View(prev_plot_spp[!is.na(prev_plot_spp$SPCD) & !is.na(prev_plot_spp$SPCD.prev),])
  
sample_prev_adult <- prev_plot_spp %>% 
  filter(!is.na(Agent)) %>% 
  group_by(SPCD, Agent) %>% 
...
First I'm just going to look at whether each species is experiencing contraction, expansion, or shift. 
...
ggdensity(comb_pcapts %>% 
       ...
       select(age, CMD), x= "CMD", color="age")
