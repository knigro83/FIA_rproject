...
pc2.shift.plot<- ggplot(pvalues.pc2, aes(x=agent, y=estimate, color = agent, fill=agent))+
  geom_point(shape = 23, size = 3.5)+
  geom_errorbar(aes(ymin=lowerci, ymax=upperci),width=0.3, size=1)+
...
ggplot(cart_positions.all, aes(x,y)) + 
  ...
  geom_path(data = for_arrows.all, 
            arrow = arrow(type = "open", length = unit(0.2, "cm")),
            alpha = 1, size=1.5,aes(col=species.name)) +
...
anova.stats.medang <- data.frame(disturbance=c("fire","insect.disease","none"), y=c(1.4,1.4,1.4),letter=c("A","B","AB"))
