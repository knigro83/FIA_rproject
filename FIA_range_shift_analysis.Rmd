...
###Niche models
```{r}
fire_15 <- env_vars_cut %>% 
  mutate(PLT_CN=env_vars$X1) %>%
  left_join(adult_data[[1]],by="PLT_CN") %>% 
  mutate(SPCD = ifelse(is.na(SPCD),0,1)) %>% 
  dplyr::rename(adult_15=SPCD) %>% 
  left_join(seedling_data[[1]],by="PLT_CN") %>% 
  mutate(SPCD = ifelse(is.na(SPCD),0,1)) %>%
  dplyr::rename(seed_15=SPCD) %>% 
  dplyr::select(c("adult_15","seed_15","MCMT.x","MSP.x","SHM.x","DD_0.x","NFFD.x","PAS.x","CMD.x","PPT_wt.x")) %>% 
  dplyr::rename(x = adult_15, y = seed_15)
View(fire_15)

sum(fire_15$adult_15)
```
...
group.hulls.args     <- list(lty = 2,col=c("red","black"))

#Group ellipses
par(mfrow=c(1,1))
plotSiberObject(fire_15_siber,
                  ax.pad = 2, 
...
legend("topright",legend= c("adult","seedling"),
...
plotGroupEllipses(fire_15_siber, n = 100, p.interval = 0.95,lty = 1, lwd = 2)
plotGroupEllipses(fire_15_siber, n = 100, p.interval = 0.95, ci.mean = T,lty = 1, lwd = 2)

group.ML <- groupMetricsML(fire_15_siber)
print(group.ML)
...
ellipses.posterior <- siberMVN(fire_15_siber, parms, priors)
...
#In this example, I will calculate the overlap between ellipses for groups 1 and 2 in community 1 (i.e. the green and yellow open circles of data).
...
# estimated using
sea.overlap <- maxLikOverlap(ellipse1, ellipse2, fire_15_siber,p.interval = NULL, n = 100)

# the overlap betweeen the corresponding 95% prediction ellipses is given by:
ellipse95.overlap <- maxLikOverlap(ellipse1, ellipse2, fire_15_siber, p.interval = 0.95, n = 100)

# so in this case, the expansion area of the seedlings would be
seedling.exp.95 <- ellipse95.overlap[2] -  ellipse95.overlap[1] #this is what I would use in the trait model 
```
