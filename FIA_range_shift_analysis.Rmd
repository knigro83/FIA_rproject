...
Let's set the minimum number of plots at 100. There are 21 species in the FIA database that have 100 or more plots in one of the 3 disturbance types with adults present, but only 12 species with seedlings present in 100+ plots per disturbance type. The species codes for those 12 species are as follows:

```{r}
#seedling sample 
sample_seeds_over100 <- sample_seeds %>% 
  filter(n_plots >= 100) %>% 
  filter(!Agent %in% c("harvest","other",'wind')) %>% 
  group_by(SPCD) %>% 
  dplyr::summarise(dist_over100 = n_distinct(Agent), disturbance = list(Agent)) %>% 
  filter(dist_over100 > 1) 
datatable(sample_seeds_over100) #there are 11 species with seedling plots > 100 in at least 2 disturbance types
```

```{r}
#make a dataframe to translate species codes to names
species.names <- data.frame(species.code = c(17,19,93,101,106,108,113,122,202,746,814), species.name = c("grand fir","subalpine fir","Engelmann spruce","whitebark pine","two needle pinyon","lodgepole pine","limber pine","ponderosa pine","Douglas-fir","trembling aspen","Gambel oak"))
species.names<- species.names %>% 
  mutate(species.code = as.numeric(species.code))

sample_seeds_over100 %>% 
  left_join(species.names, by=c("SPCD"="species.code")) %>% 
  datatable()

sample_seeds %>% 
  filter(SPCD %in% c(17,106,113,122))
```


```{r, echo=FALSE,warning=FALSE}
#filter sample size table to species with adults in at least 100 plots 
sample_adults_over100_cmb <- adult_sample_cmbsurv %>% 
  filter(n_plots >= 100) %>% 
  filter(!Agent %in% c("harvest","other",'wind')) %>% 
  group_by(SPCD) %>%
  dplyr::summarise(dist_over100 = n_distinct(Agent),disturbance = list(Agent)) %>% 
...
datatable(sample_adults_over100_cmb)
```
...
sp_codes <- as.character(unique(sample_seeds_over100$SPCD))
...
pc1.diffs.sep <- pc1.diffs.table %>% 
  ...
  filter(!(agent == "fire" & species.name %in% c("limber pine","two needle pinyon","grand fir"))) #%>% 
  #filter(!(agent == 'insect.disease' & species.name == 'ponderosa pine'))
```
...
Difference data on the 5th quantile of PC1 for undisturbed plots and the 95th quantile of PC1 for burned plots is not normal. 
...
#need to do Wilcoxon Signed Rank Test for 5th percentile of nonebecause data is not normal 
...
pc1.diffs.fullset<- pc1.diffs.sep %>% 
  filter(!species.name %in% c("limber pine","two needle pinyon","grand fir"))

par(mfrow=c(1,1))

#check for significant differences in 5th percentile by disturbance type
pc1.mod5 <- lm(quant_5 ~ agent, data=pc1.diffs.fullset)
qqPlot(pc1.mod5)
leveneTest(quant_5 ~ agent, data=pc1.diffs.fullset)
anova(pc1.mod5)
#check for significant differences in 50th percentile by disturbance type
pc1.mod50 <- lm(quant_50 ~ agent, data=pc1.diffs.fullset)
qqPlot(pc1.mod50)
leveneTest(quant_50 ~ agent, data=pc1.diffs.fullset)
anova(pc1.mod50)
#check for significant differences in 95th percentile by disturbance type
pc1.mod95 <- lm(quant_95 ~ agent, data=pc1.diffs.fullset)
qqPlot(pc1.mod95)
leveneTest(quant_95 ~ agent, data=pc1.diffs.fullset)
anova(pc1.mod95)
```
...
}   ##most but not all have equal variance between adult and seedling data
```
...
#this table shows sample size for seedling data that didn't pass normality test. Lowest is 170 (aspen in insect.disease) so i think this is big enough to be fine.   
shapiro.results.s %>% 
  filter(p<0.05) %>% 
  left_join(data.names, by='num') %>% 
  separate(name,sep="_",into=c("species","agent","var")) %>% 
...
#siber.objs[which(names(siber.objs) %in% c("17_fire_cvars","106_fire_cvars","113_fire_cvars","122_insect.disease_cvars"))]<-NULL
...
```{r}
##save model output
# saveRDS(iso_data, file="iso_data.rds")
# saveRDS(siber.objs, file="siber.objs.rds")
# saveRDS(ellipses.posterior.list, file="ellipses.posterior.list.rds")
# saveRDS(SEA.B.list, file="SEA.B.list.rds")
```

```{r}
#read in siber files
iso_data <- readRDS("iso_data.rds")
siber.objs <- readRDS("siber.objs.rds")
ellipses.posterior.list<- readRDS("ellipses.posterior.list.rds")
SEA.B.list <- readRDS("SEA.B.list.rds")
```
...
saveRDS(overlap.list, file = "overlap.list.update.nov.RDS")
...
overlap.list <- readRDS("overlap.list.nov.RDS")
...
overlap.table <- as.data.frame(matrix(unlist(overlap.summary),nrow = 29, ncol=5,byrow=TRUE))  
...
```{r}
#some plots
ggplot(overlap.table,aes(x=species.name,y=mean.exp.prop.adult,fill=disturbance))+
  geom_bar(stat='identity',position='dodge')+
  theme(axis.text.x = element_text(angle=45,vjust=0.9,hjust=0.8))

ggplot(overlap.table,aes(x=species.name,y=mean.contr.prop.adult,fill=disturbance))+
  geom_bar(stat='identity',position='dodge')+
  theme(axis.text.x = element_text(angle=45,vjust=0.9,hjust=0.8))


expansion.plot <- ggplot(overlap.table,aes(x=disturbance,y=mean.exp.prop.adult))+
...
contraction.plot <- ggplot(overlap.table,aes(x=disturbance,y=mean.contr.prop.adult))+
  geom_boxplot(fill="gray")+
  geom_jitter(height=0,width=0.2, aes(color=species.name),pch=15,size=2)+
  scale_color_manual(values=c("#c9ce4e","#8b4cca","#6ccb5b","#c74a90","#8fcdb3","#d0503b","#708fc2","#c39553","#483262",
"#4a633f","#cd9bb5","#6c3631"),name="Species")+
  xlab("")+
  ylab("Proportion Contracted")+
  theme_bw()+
...
ggplot(overlap.table %>% 
  ...
  ,aes(x=species.name,y=proportion,fill=shift))+
  geom_boxplot()+
  xlab("")+
...
ggplot(overlap.table %>% 
  pivot_longer(c(mean.exp.prop.adult,mean.contr.prop.adult),names_to = "shift",values_to = "proportion")
  ,aes(x=shift,y=proportion,fill=shift))+
  geom_boxplot()+
  xlab("")+
  ylab("Mean Proportion Shift")+
  scale_fill_manual(values=c("darkorange1","steelblue"),labels=c("Contraction", "Expansion"),name="Shift Type")+
...
Now I will look at differences between disturbances in the amount of contraction/expansion. 

```{r}
dist.exp.anova <- aov(log(mean.exp.prop.adult)~disturbance,data=overlap.table)
par(mfrow=c(2,2))
plot(dist.exp.anova)
summary(dist.exp.anova) #no significant difference between disturbances in expansion

dist.contr.anova<- aov(log(mean.contr.prop.adult)~disturbance, data=overlap.table)
plot(dist.contr.anova)
summary(dist.contr.anova)#no difference between disturbances for contractions

hist(overlap.table$mean.exp.prop.adult) #not normal
hist(overlap.table$mean.contr.prop.adult)
var.test(overlap.table$mean.exp.prop.adult, overlap.table$mean.contr.prop.adult, alternative = "two.sided")
wilcox.test(overlap.table$mean.exp.prop.adult, overlap.table$mean.contr.prop.adult)
...
cor.test(overlap.table$mean.exp.prop.adult,overlap.table$mean.contr.prop.adult)
plot(overlap.table$mean.exp.prop.adult,overlap.table$mean.contr.prop.adult)
...
#shade tolerance
exp.shade.mod<- lm(log(mean.exp.prop.adult)~shade.tolerance*disturbance,data=niche.shift.trait.table %>% filter(!species.name %in% c("grand fir","two needle pinyon","limber pine","ponderosa pine")))
par(mfrow=c(2,2))
plot(exp.shade.mod)
...
exp.weight.mod<- lm(log(mean.exp.prop.adult)~mean.weight.mg*disturbance,data=niche.shift.trait.table %>% filter(!species.name %in% c("grand fir","two needle pinyon","limber pine","ponderosa pine","Gambel oak")))
par(mfrow=c(2,2))
plot(exp.weight.mod)
...
#seed weight
#take out pinyon, gambel oak and juniper because they are primarily dispersed by animals
contr.weight.mod<- lm(log(mean.contr.prop.adult)~mean.weight.mg*disturbance,data=niche.shift.trait.table %>% filter(! species %in% c(106,814,63)))
par(mfrow=c(2,2))
...
colnames(pc1means.table)[1]<-"pc1mean"

ggplot(pc1means.table,aes(x=pc1mean,y=rep(1,length(pc1.means)),color=factor(species.name)))+
  geom_point()

ggplot(pc1means.table,aes(x=factor(species.name),y=pc1mean))+
  geom_boxplot()+
  coord_flip()
...
niche.shift.trait.table<-
  niche.shift.trait.table %>% 
  left_join(pc1means.table,by=c("species.name","disturbance","extra"))

pc1.exp.mod <- lm(log(mean.exp.prop.adult)~ pc1mean*disturbance, data=niche.shift.trait.table)
plot(pc1.exp.mod)
Anova(pc1.exp.mod, type='3')
summary(pc1.exp.mod)
emtrends(pc1.exp.mod, pairwise ~ disturbance, var = "pc1mean")


pc1.exp.plot <- ggplot(data=niche.shift.trait.table,aes(x=pc1mean,y=log(mean.exp.prop.adult),color=disturbance))+
  geom_point(size=3)+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  theme_bw()+
...
pc1.contr.mod <- lm(log(mean.contr.prop.adult)~ pc1mean*disturbance, data=niche.shift.trait.table)
plot(pc1.contr.mod)
Anova(pc1.contr.mod, type='3')
summary(pc1.contr.mod)
emtrends(pc1.contr.mod, pairwise ~ disturbance, var = "pc1mean")


pc1.contr.plot<- ggplot(data=niche.shift.trait.table,aes(x=pc1mean,y=log(mean.contr.prop.adult),color=disturbance))+
  geom_point(size=3)+
  geom_smooth(method = "lm", se = FALSE, fullrange = T, size=1)+
  theme_bw()+
