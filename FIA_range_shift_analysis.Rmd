...
wind<- read_csv("wind.plots2020.csv")
...
##this table shows the number of seedling plots surveyed each year. We will just use those surveyed form 2009-2018
left_join(seeds, dist, by="PLT_CN") %>% group_by(MEASYEAR) %>% tally() %>% datatable()

dist_seeds_joined <- left_join(seeds, dist, by="PLT_CN") %>% 
  filter(MEASYEAR > 2006)
...
no.prev.surv.plots/(prev.surv.plots+no.prev.surv.plots) #58% of plots in analysis only had one survey
...
sample_seeds <- dist_seeds_joined %>% 
  filter(!is.na(Agent)) %>%
  filter(MEASYEAR > 2006) %>% #only look at seedling data from plots surveyed in the later chunk of surveys
  group_by(SPCD, Agent) %>% 
  dplyr::summarise(n_plots = n_distinct(PLT_CN))
...
species.names <- data.frame(species.code = c(15,17,19,93,101,106,108,113,122,133,202,242,746,814), species.name = c("white fir","grand fir","subalpine fir","Engelmann spruce","whitebark pine","two needle pinyon","lodgepole pine","limber pine","ponderosa pine","singleleaf pinyon","Douglas-fir","western redcedar","trembling aspen","Gambel oak"))
...
There are 14 species in the FIA database that have 60 or more plots in at least 2 of the 3 disturbance types with seedlings present. 
...
```{r}
#for some reason there are 3 plots in the environmental data that are not in the disturbance data. I think it has to do with how I eliminated plots with artificial regen. 
#let's just eliminate these 3 plots from the environmental data
env_vars <- env_vars %>% 
...
```{r, echo=FALSE}
env_vars %>% 
  dplyr::select(-PLT_CN,-FIA_slope,-FIA_elev,-foldedaspect,-TD) %>% 
...
env_vars_cut <- env_vars %>% 
  dplyr::select(-PLT_CN,-FIA_slope,-FIA_elev,-foldedaspect,-TD, -ppt, -tmean, -tmax, - tmin, -vpdmin, -vpdmax, -MWMT, -AHM, -DD5, -bFFP, -eFFP, -EMT, -Eref, -Tave_wt, -Tave_sm, -PPT_sm, -awc.total)

env_vars_cut_alt <- env_vars %>% 
  dplyr::select(-PLT_CN,-FIA_slope,-FIA_elev,-foldedaspect,-TD, -ppt, -tmean, -tmax, - tmin, -vpdmin, -vpdmax, -MWMT, -AHM, -DD5, -bFFP, -eFFP, -EMT, -Eref, -Tave_wt, -Tave_sm, -PPT_sm, -awc.total,-CMD, -MCMT)
...
# pdf(file="C:/Users/Katie/Google Drive/FIA project/FIA_rproject/pre_trans_pairs.pdf")
# 
...
#saveRDS(clim_pca, file = "FIA_pca_nov.RDS")
clim_pca <- readRDS("FIA_pca_nov.RDS")
...
# as.data.frame(alt_clim_pca$rotation[,1:2]) %>% 
#   tibble::rownames_to_column() %>% 
#   arrange(desc(abs(PC1)))
```
...
#rownames(clim_pca$rotation) <- c("Mean temp coldest month","# frost free days","climate moisture deficit","summer precip","summer temp/precip","winter precip","precip as snow","days below freezing")

autoplot(clim_pca,x=1,y=2,colour="grey",loadings.colour="red", loadings=T, loadings.label=T, loadings.label.colour="black", scale=0, loadings.label.vjust=c(-.5,-.3,-.5,-.3,-.4,1.1,1.4,0), loadings.label.hjust=c(0,1,0,-.1,1.1,-.1,0,-.1), loadings.label.size=7, alpha=0.5)+
  geom_hline(yintercept=0,linetype="dashed")+geom_vline(xintercept=0,linetype="dashed")+
  geom_abline(intercept=0, slope = 1, linetype="dashed")+
  geom_abline(intercept=0, slope = -1, linetype="dashed")+
  annotate("text",x=-0.2,y=3.6,label="hot/wet",size=6,angle=90)+
  annotate(geom="text",x=5.5,y=0.2,label="cold/wet",size=6)+
  annotate(geom="text",x=-0.2,y=-2.5,label="cold/dry",size=6,angle=90)+
  annotate(geom="text",x=-6.7,y= 0.2,label="hot/dry",size=6)+
  annotate(geom="text",x=3.4,y= 3.8,label="wet",size=6, angle=45)+
  annotate(geom="text",x=3,y= -2.7,label="cold",size=6, angle=313)+
  annotate(geom="text",x=-3.2,y= -2.8,label="dry",size=6, angle=45)+
  annotate(geom="text",x=-3.4,y= 3.7,label="hot",size=6, angle=312)+
  theme_bw()+
  theme(axis.text = element_text(color="black",size=18),text = element_text(size=18),legend.text = element_text(size=18))

# autoplot(alt_clim_pca,x=1,y=2,colour="grey",loadings.colour="black", loadings=T, loadings.label=T, loadings.label.colour="black", scale=0, loadings.label.vjust=-1.5, loadings.label.size=8, alpha=0.5)+
#   theme_bw()+
#   theme(axis.text = element_text(color="black",size=18),text = element_text(size=18),legend.text = element_text(size=18))

 pdf("C:/Users/Katie/Google Drive/FIA project/FIA_rproject/figures/FIA_pca.pdf", width = 13, height = 7)
 ...
 autoplot(clim_pca,x=1,y=2,colour="grey",loadings.colour="red", loadings=T, loadings.label=T, loadings.label.colour="black", scale=0, loadings.label.vjust=c(-.5,-.3,-.5,-.3,-.4,1.1,1.4,0), loadings.label.hjust=c(0,1,0,-.1,1.1,-.1,0,-.1), loadings.label.size=7, alpha=0.5)+
  ...
  annotate(geom="text",x=-6.7,y= 0.2,label="hot/dry",size=6)+
  annotate(geom="text",x=3.4,y= 3.8,label="wet",size=6, angle=45)+
  annotate(geom="text",x=3,y= -2.7,label="cold",size=6, angle=315)+
  annotate(geom="text",x=-3.2,y= -2.8,label="dry",size=6, angle=45)+
  annotate(geom="text",x=-3.4,y= 3.7,label="hot",size=6, angle=315)+
  #xlim(c(-7,7))+ylim(c(-4,4))+
  theme_bw()+
  theme(axis.text = element_text(color="black",size=18),text = element_text(size=18),legend.text = element_text(size=18))
...
#calculate difference in 10th, 50th and 90th percentiles in PC1 position between adults and seedlings of each species and disturbance type 
pc1.diffs.table.quant90 <- data.frame(uid = names(adult_data_pcapts))
for(i in c(0.1, 0.5, 0.90)){
diffs <- get_diff(adult_data_pcapts, seedling_data_pcapts, quo(PC1), i) 
  pc1.diffs.table.quant90 <- pc1.diffs.table.quant90 %>% 
    left_join(diffs, by = "uid")
}

#get dataframe in order for analysis and plotting
pc1.diffs.sep.quant90 <- pc1.diffs.table.quant90 %>% 
  separate(uid, c("species","agent"),sep="_",remove=FALSE, extra="drop") %>% 
  mutate(species = as.numeric(species)) %>% 
  left_join(species.names,by=c("species" = "species.code")) %>% 
  filter(!(agent == "fire" & species.name %in% insectonlyspp)) 
...
```{r, eval=FALSE,echo=FALSE, eval=FALSE, results="hide"}
##checking for normality
fire.diffs.pc1.quant90<- pc1.diffs.sep.quant90 %>% 
  filter(agent == "fire") 
id.diffs.pc1.quant90<- pc1.diffs.sep.quant90 %>% 
  filter(agent == "insect.disease") 
none.diffs.pc1.quant90<- pc1.diffs.sep.quant90 %>% 
  filter(agent == "none")

#fire 10th quantile 
ggqqplot(fire.diffs.pc1.quant90$quant_10)
shapiro.test(fire.diffs.pc1.quant90$quant_10)

#fire 50th quantile
ggqqplot(fire.diffs.pc1.quant90$quant_50)
shapiro.test(fire.diffs.pc1.quant90$quant_50)

#fire 90th quantile 
ggqqplot(fire.diffs.pc1.quant90$quant_90)
shapiro.test(fire.diffs.pc1.quant90$quant_90)

#id 10th quantile
ggqqplot(id.diffs.pc1.quant90$quant_10)
shapiro.test(id.diffs.pc1.quant90$quant_10)

#id 50th quantile
ggqqplot(id.diffs.pc1.quant90$quant_50)
shapiro.test(id.diffs.pc1.quant90$quant_50)

#id 90th quantile -- NOT NORMAL
ggqqplot(id.diffs.pc1.quant90$quant_90)
shapiro.test(id.diffs.pc1.quant90$quant_90)

#none 10th quantile 
ggqqplot(none.diffs.pc1.quant90$quant_10)
shapiro.test(none.diffs.pc1.quant90$quant_10)

#none 50th quantile
ggqqplot(none.diffs.pc1.quant90$quant_50)
shapiro.test(none.diffs.pc1.quant90$quant_50)

#none 90th quantile
ggqqplot(none.diffs.pc1.quant90$quant_90)
shapiro.test(none.diffs.pc1.quant90$quant_90)
...
```{r,eval=FALSE,echo=FALSE, results="hide"}
#test for significant shifts from zero for 90th and 10th quantiles
...
#'
#' @examples
get_pvalues_90 <- function(pc1.diffs.sep){
agent.list = unique(pc1.diffs.sep$agent)
quant.list = c(10, 50, 90)
pc1.p.table = data.frame()
for(i in 1:length(agent.list)){
  cat(i, "\n")
  for(j in 1:length(quant.list)){
...
#this call to options will tell me where the warnings occur. In this case, for the comparison of medians in the fire plots and harvest plots, the exact p-value cannot be calculated because there are values that are exactly equal to 0 in the data. I think this is ok.  
options(warn = 1)
pvalues.pc1.quant90 <- get_pvalues_90(pc1.diffs.sep.quant90)
pvalues.pc1.quant90
pvalues.pc1.quant90<- pvalues.pc1.quant90 %>% 
  mutate(wilcoxon = "n")

#need to do Wilcoxon Signed Rank Test for 90th percentile of insect/disease because data is not normal 
id90.test <- pc1.diffs.sep.quant90 %>% 
  filter(agent == "insect.disease") %>% 
  pull("quant_90") %>% 
  wilcox.test(y = NULL, alternative = "two.sided", mu = 0, conf.int = TRUE, conf.level = 0.95)

id90.p.df <- data.frame(agent = "insect.disease", quant = "quant_90", p.value = id90.test$p.value, estimate = id90.test$estimate, lowerci = id90.test$conf.int[1], upperci = id90.test$conf.int[2], t=id90.test$statistic, df=NA, wilcoxon="y")

##replace 90th percentile of insect/disease with wilcoxon test values
pvalues.pc1.quant90<- pvalues.pc1.quant90 %>% 
                rows_update(id90.p.df,by=c("agent","quant"))
pvalues.pc1.quant90
```
...
```{r, eval=FALSE, echo=FALSE}
#make data long for plotting
pc1.diffs.long.quant90 <- pc1.diffs.sep.quant90 %>% 
  pivot_longer(cols = c(quant_10, quant_50, quant_90), names_to = "quant", values_to = "diff")

ggplot(pc1.diffs.long.quant90, aes(x=agent, y=diff))+
  geom_boxplot(fill="lightgrey")+
  geom_jitter(height=0,width=0.2, aes(color=species.name))+
  facet_wrap(~quant)+
  theme_bw()+
...
##dataframe for stars
pvalues.pc1.quant90<- pvalues.pc1.quant90 %>% 
  mutate(sym.pos = ifelse(p.value<0.05,0.17,NA))

quant.labs = c("10th quantile","50th quantile","90th quantile") 
names(quant.labs)= c("quant_10","quant_50","quant_90")

ggplot(pvalues.pc1.quant90, aes(x=agent, y=estimate, color = quant, fill=quant))+
  geom_point(shape = 23, size = 3.5)+
  geom_errorbar(aes(ymin=lowerci, ymax=upperci),width=0.3, size=1)+
  scale_color_manual(values = c("#d7191c", "#fdae61","#2b83ba"))+
  scale_fill_manual(values = c("#d7191c", "#fdae61","#2b83ba"))+
  geom_text(data = subset(pvalues.pc1.quant90, !is.na(sym.pos)), aes(x=agent, y=sym.pos), label="*", color="black",size=8)+
  geom_hline(yintercept=0, color="black", lty=2, size=1)+
facet_wrap(~ quant, labeller = labeller(quant=quant.labs))+
  ggtitle("Shifts on PC1")+
  ylab("Difference on PC1")+
...
```{r, eval=FALSE, echo=FALSE, results='hide'}
#calculate difference in 10th, 50th and 90th percentiles in pc2 position between adults and seedlings of each species and disturbance type 
pc2.diffs.table.quant90 <- data.frame(uid = names(adult_data_pcapts))
for(i in c(0.1, 0.5, 0.9)){
diffs <- get_diff(adult_data_pcapts, seedling_data_pcapts, quo(PC2), i) 
  pc2.diffs.table.quant90 <- pc2.diffs.table.quant90 %>% 
    left_join(diffs, by = "uid")
}
pc2.diffs.table.quant90

#get dataframe in order for analysis and plotting
pc2.diffs.sep.quant90 <- pc2.diffs.table.quant90 %>% 
  separate(uid, c("species","agent"),sep="_",remove=FALSE, extra="drop") %>% 
  mutate(species = as.numeric(species)) %>% 
  left_join(species.names,by=c("species" = "species.code")) %>% 
  filter(!(agent == "fire" & species.name %in% insectonlyspp)) %>% 
...
5th quantile shifts in undisturbed plots are non-normal

```{r,echo=FALSE,eval=FALSE, results='hide'}
##checking for normality
fire.diffs.pc2.quant90<- pc2.diffs.sep.quant90 %>% 
  filter(agent == "fire") 
id.diffs.pc2.quant90<- pc2.diffs.sep.quant90 %>% 
  filter(agent == "insect.disease") 
none.diffs.pc2.quant90<- pc2.diffs.sep.quant90 %>% 
  filter(agent == "none")

#fire 5th quantile
ggqqplot(fire.diffs.pc2.quant90$quant_10)
shapiro.test(fire.diffs.pc2.quant90$quant_10)

#fire 50th quantile 
ggqqplot(fire.diffs.pc2.quant90$quant_50)
shapiro.test(fire.diffs.pc2.quant90$quant_50)

#fire 95th quantile
ggqqplot(fire.diffs.pc2.quant90$quant_90)
shapiro.test(fire.diffs.pc2.quant90$quant_90)

#id 10th quantile -- NOT NORMAL
ggqqplot(id.diffs.pc2.quant90$quant_10)
shapiro.test(id.diffs.pc2.quant90$quant_10)

#id 50th quantile
ggqqplot(id.diffs.pc2.quant90$quant_50)
shapiro.test(id.diffs.pc2.quant90$quant_50)

#id 90th quantile 
ggqqplot(id.diffs.pc2.quant90$quant_90)
shapiro.test(id.diffs.pc2.quant90$quant_90)

#none 10th quantile 
ggqqplot(none.diffs.pc2.quant90$quant_10)
shapiro.test(none.diffs.pc2.quant90$quant_10)

#none 50th quantile
ggqqplot(none.diffs.pc2.quant90$quant_50)
shapiro.test(none.diffs.pc2.quant90$quant_50)

#none 90th quantile
ggqqplot(none.diffs.pc2.quant90$quant_90)
shapiro.test(none.diffs.pc2.quant90$quant_90)
```
...
# #95th none
# pc2.none95.test <- pc2.diffs.sep %>% 
#   filter(agent == "none") %>% 
#   pull("quant_95") %>% 
#   wilcox.test(y = NULL, alternative = "two.sided", mu = 0, conf.int = TRUE, conf.level = 0.95)
# 
# pc2.none95.p.df <- data.frame(agent = "none", quant = "quant_95", p.value = pc2.none95.test$p.value, estimate = pc2.none95.test$estimate, lowerci = pc2.none95.test$conf.int[1], upperci = pc2.none95.test$conf.int[2],t=pc2.none95.test$statistic,df=NA,wilcoxon="y")
# 
# #95th ID
...
#   wilcox.test(y = NULL, alternative = "two.sided", mu = 0, conf.int = TRUE, conf.level = 0.95)
# 
# pc2.ID95.p.df <- data.frame(agent = "insect.disease", quant = "quant_95", p.value = pc2.ID95.test$p.value, estimate = pc2.ID95.test$estimate, lowerci = pc2.ID95.test$conf.int[1], upperci = pc2.ID95.test$conf.int[2],t=pc2.ID95.test$statistic,df=NA,wilcoxon="y")
...
```{r,eval=FALSE, echo=FALSE, results='hide'}
#perform T-tests
pvalues.pc2.quant90 <- get_pvalues_90(pc2.diffs.sep.quant90)
pvalues.pc2.quant90
pvalues.pc2.quant90<- pvalues.pc2.quant90 %>% 
  mutate(wilcoxon="n")

#perform wilcoxon tests for non-normal data
#10th id
pc2.id10.test <- pc2.diffs.sep.quant90 %>% 
  filter(agent == "insect.disease") %>% 
  pull("quant_10") %>% 
  wilcox.test(y = NULL, alternative = "two.sided", mu = 0, conf.int = TRUE, conf.level = 0.95)

pc2.id10.p.df <- data.frame(agent = "insect.disease", quant = "quant_10", p.value = pc2.id10.test$p.value, estimate = pc2.id10.test$estimate, lowerci = pc2.id10.test$conf.int[1], upperci = pc2.id10.test$conf.int[2],t=pc2.id10.test$statistic,df=NA,wilcoxon="y")

##replace t-test with wilcoxon test values
pvalues.pc2.quant90<- pvalues.pc2.quant90 %>% 
                rows_update(pc2.id10.p.df, by=c("agent","quant"))
pvalues.pc2.quant90
```
...
pc2.shift.plot<- ggplot(pvalues.pc2 %>%  mutate(across(quant,factor,levels=c("quant_95","quant_50","quant_5"))), aes(x=agent, y=estimate, color = agent, fill=agent))+
  ...
  ylim(c(-0.15,0.16))+
  facet_grid(rows= vars(quant), labeller= labeller(quant=quant.labs))+
...
(pvalues.pc2 %>%  mutate(across(quant,factor,levels=c("quant_95","quant_50","quant_5"))))$quant
...
```{r, echo=FALSE}
#make data long for plotting
pc2.diffs.long.quant90 <- pc2.diffs.sep.quant90 %>% 
  pivot_longer(cols = c(quant_10, quant_50, quant_90), names_to = "quant", values_to = "diff")

ggplot(pc2.diffs.long.quant90, aes(x=agent, y=diff))+
  geom_boxplot(fill="lightgrey")+
  geom_jitter(height=0,width=0.2, aes(color=species.name))+
  facet_wrap(~quant)+
  theme_bw()+
...
ggplot(pvalues.pc2.quant90, aes(x=agent, y=estimate, color = quant, fill=quant))+
  geom_point(shape = 23, size = 3.5)+
  geom_errorbar(aes(ymin=lowerci, ymax=upperci),width=0.3, size=1)+
  scale_color_manual(values = c("tan4", "#fdae61","turquoise"))+
  scale_fill_manual(values = c("tan4", "#fdae61","turquoise"))+
  geom_text(data = subset(pvalues.pc2.quant90, !is.na(sym.pos)), aes(x=agent, y=sym.pos), label="*", color="black",size=8)+
  geom_hline(yintercept=0, color="black", size=1, lty=2)+
facet_wrap(~ quant, labeller= labeller(quant=quant.labs))+
  ggtitle("Shifts on PC2")+
  xlab("")+
...
pc1.ttest.pvalues.sep <- pc1.ttest.pvalues %>% 
  ...
  mutate(species = as.numeric(species)) %>% 
  left_join(species.names, by = c("species" = "species.code")) %>%   mutate(sym.pos = ifelse(p.value<0.05, 0.35, NA)) %>% 
  filter(!(agent == "fire" & species.name %in% insectonlyspp)) %>% 
  filter(!species.name == "singleleaf pinyon")
```
...
ggplot(pc1.ttest.pvalues.sep, aes(x = agent, y = diff.means, fill = agent))+
  ...
  theme_bw()+
  theme(axis.text.x = element_blank(), axis.text=element_text(color="black", size=14), legend.text = element_text(color="black", size=14), text=element_text(color="black", size=14), strip.text = element_text(color="black", size=11), legend.position = c(0.9, 0),legend.justification = c(1, -0.3))
```
...
ggplot(pc2.ttest.pvalues.sep, aes(x = agent, y = diff.means, fill = agent))+
  ...
  labs(fill = "Disturbance Agent")+
  theme_bw()+
  theme(axis.text.x = element_blank(), axis.text=element_text(color="black",size=14), text=element_text(color="black",size=14), legend.text = element_text(color="black",size=14), strip.text = element_text(color="black",size=11),legend.position = c(0.9,0), legend.justification = c(1,-0.3))
```
...
# siber.objs[which(names(siber.objs) %in% c("15_fire_cvars","17_fire_cvars","106_fire_cvars","113_fire_cvars","133_fire_cvars","242_fire_cvars"))]<-NULL
# 
# names(siber.objs)
```
...
```{r, echo=FALSE}
##save model output
 # saveRDS(iso_data, file="iso_data.14spp.nov.rds")
 # saveRDS(siber.objs, file="siber.objs.14spp.nov.rds")
 # saveRDS(ellipses.posterior.list, file="ellipses.posterior.list.14spp.nov.rds")
 # saveRDS(SEA.B.list, file="SEA.B.list.14spp.nov.rds")
```

```{r}
#read in siber files
iso_data <- readRDS("iso_data.14spp.nov.rds")
siber.objs <- readRDS("siber.objs.14spp.nov.rds")
ellipses.posterior.list<- readRDS("ellipses.posterior.list.14spp.nov.rds")
SEA.B.list <- readRDS("SEA.B.list.14spp.nov.rds")
```
...
disturbance.colors
color.list = c(disturbance.colors[2:3],disturbance.colors[2:3],rep(disturbance.colors,3),disturbance.colors[2:3],disturbance.colors,disturbance.colors[2:3],disturbance.colors,disturbance.colors[2:3],disturbance.colors,disturbance.colors[2:3],rep(disturbance.colors,2))

# par(mfrow=c(1,1))
# for(i in 1:length(siber.objs)){
#   png(filename = paste("C:/Users/Katie/Google Drive/FIA project/FIA_rproject/siberplots/",names(siber.objs)[i],".jpeg",sep=""))
...
# 
# saveRDS(overlap.list, file = "overlap.list.14spp.nov.first13.RDS")
# 
# overlap.list <- readRDS("overlap.list.14spp.nov.first13.RDS")
# 
# for(i in 14:length(ellipses.posterior.list)){
#     cat(i, "\n")
# temp.overlap <- bayesianOverlap("1.1","1.2",ellipses.posterior.list[[i]],draws=NULL,p.interval=0.95)
# 
# temp.overlap$exp.prop.adult = (temp.overlap$area2 - temp.overlap$overlap)/temp.overlap$area1
...
# names(overlap.list)<- names(ellipses.posterior.list)
# 
# saveRDS(overlap.list, file = "overlap.list.14spp.nov.RDS")

```

```{r}
#calculate niche areas and overlap for 90% confidence
##this takes ~10 hours 
# overlap.list.90<-list()
# for(i in 1:length(ellipses.posterior.list)){
#     cat(i, "\n")
# temp.overlap <- bayesianOverlap("1.1","1.2",ellipses.posterior.list[[i]],draws=NULL,p.interval=0.90)
# 
# temp.overlap$exp.prop.adult = (temp.overlap$area2 - temp.overlap$overlap)/temp.overlap$area1
# 
# temp.overlap$contr.prop.adult = (temp.overlap$area1 - temp.overlap$overlap)/temp.overlap$area1
# 
# overlap.list.90[[i]] <- temp.overlap
# }
# names(overlap.list.90)<- names(ellipses.posterior.list)
# 
# saveRDS(overlap.list.90, file = "overlap.list.90.nov.RDS")

```

```{r}
#read in overlap data
overlap.list <- readRDS("overlap.list.14spp.nov.RDS")
names(overlap.list) <- names(ellipses.posterior.list)
...
overlap.table <- as.data.frame(matrix(unlist(overlap.summary),nrow = 36, ncol=5,byrow=TRUE))  
...
```{r}
#some plots
ggplot(overlap.table %>% filter(!(disturbance == 'fire' & species.name %in% c("limber pine","grand fir","two needle pinyon"))),aes(x=species.name,y=mean.exp.prop.adult,fill=disturbance))+
  geom_bar(stat='identity',position='dodge')+
  scale_fill_manual(values=disturbance.colors)+
  theme(axis.text.x = element_text(angle=45,vjust=0.9,hjust=0.8))

ggplot(overlap.table %>% filter(!(disturbance == 'fire' & species.name %in% c("limber pine","grand fir","two needle pinyon"))),aes(x=species.name,y=mean.contr.prop.adult,fill=disturbance))+
  geom_bar(stat='identity',position='dodge')+
  scale_fill_manual(values=disturbance.colors)+
  theme(axis.text.x = element_text(angle=45,vjust=0.9,hjust=0.8))
...
#for 8 species
diff.plot<- ggplot(overlap.table %>% filter(!(species.name %in% insectonlyspp)),aes(x=species.name,y=diff,fill=disturbance))+
  geom_bar(stat='identity',position='dodge')+
  scale_fill_manual(values=disturbance.colors)+
  scale_x_discrete(position='top',name="")+
  ylab("Expansion - Contraction")+
  theme_bw()+
  theme(axis.text.x = element_text(angle=25,vjust=0,hjust=0),axis.text = element_text(size=24),legend.text = element_text(size=24),legend.title = element_text(size=24),legend.position = 'none',axis.title = element_text(size=24),plot.margin=margin(0,3.8,.1,.1,"cm"))
...
expansion.plot <- ggplot(overlap.table %>% filter(!(disturbance == "fire" & species.name %in% c("limber pine", "grand fir","two needle pinyon"))),aes(x=disturbance,y=mean.exp.prop.adult))+
...
contraction.plot <- ggplot(overlap.table %>% filter(!(disturbance == "fire" & species.name %in% c("limber pine", "grand fir","two needle pinyon"))),aes(x=disturbance,y=mean.contr.prop.adult))+
  geom_boxplot(fill="gray")+
  geom_jitter(height=0,width=0.2, aes(color=species.name),pch=15,size=2)+
  scale_color_manual(values=c("#d55f90","#5aa554","#ad74d6","#9ab13f","#6070d5","#cd9337","#563686","#45c097","#8b2965","#9b8a40","#ca71c0","#b95336","#628ed6","#ba4758"),name="Species")+
  xlab("")+
...
species.numbers <- data.frame(species.name = c("two needle pinyon","Gambel oak","ponderosa pine","white fir","trembling aspen","Douglas-fir","western redcedar","limber pine","grand fir","lodgepole pine","Engelmann spruce","subalpine fir","whitebark pine"), species.number = seq(1,13,1))
...
ggplot()+
    geom_polygon(data=df.triangle,aes(y=y,x=x,group))


arrowsplot1<- ggplot(cart_positions.all, aes(x,y)) +
  geom_polygon(data=df.top.triangle, aes(x,y),fill=rect.colors[2],alpha=0.5)+
  geom_polygon(data=df.right.triangle, aes(x,y),fill=rect.colors[4],alpha=0.5)+
  geom_polygon(data=df.bottom.triangle, aes(x,y),fill=rect.colors[3],alpha=0.5)+
  geom_polygon(data=df.left.triangle, aes(x,y),fill=rect.colors[1],alpha=0.5)+
...
arrow_extremes <- for_arrows.all %>% 
  filter(!x==0) %>% 
  group_by(disturbance) %>% 
  dplyr::summarise(min.x = min(x), min.y=min(y), max.y=max(y))

top_points<- for_arrows.all %>% 
  filter(!x==0) %>% 
  filter(disturbance=="insect.disease" & species.name %in% c("limber pine")|
           disturbance == "fire" & species.name %in% c("lodgepole pine")|
           disturbance == "none" & species.name %in% c("whitebark pine")) %>% 
  dplyr::rename(c("xtop" = "x", "ytop" = "y"))

bottom_points<- for_arrows.all %>% 
  filter(!x==0) %>% 
  filter(disturbance=="insect.disease" & species.name %in% c("whitebark pine")|
           disturbance == "fire" & species.name %in% c("whitebark pine")|
           disturbance == "none" & species.name %in% c("white fir")) %>% 
  dplyr::rename(c("xbot" = "x", "ybot" = "y"))

extreme_points <- top_points %>% 
  left_join(bottom_points, by="disturbance")


ggplot(extreme_points)+
  geom_curve(aes(x=xtop, y=ytop,xend=xbot,yend=ybot,color=disturbance), curvature = -0.5)+
  scale_color_manual(values=disturbance.colors)+
...
#distance model 
distmodmed <- aov(log(medDist) ~ factor(disturbance), data=median_vectors_all)
par(mfrow=c(2,2))
plot(distmodmed)
summary(distmodmed)
TukeyHSD(distmodmed)

anova.stats.meddist <- data.frame(disturbance=c("fire","insect.disease","none"), y=c(0.37,0.37,0.37),letter=c("AB","A","B"))

ggplot(median_vectors_all, aes(x=disturbance, y=medDist, fill=disturbance))+
  geom_boxplot()+
  ylab("Median distance")+
  xlab("")+
  scale_fill_manual(values=disturbance.colors)+
  geom_text(data= anova.stats.meddist, aes(x=disturbance,y=y,label=letter),size=8)+
  theme_bw()+
  theme(text=element_text(size=26, color="black"),axis.text = element_text(color="black"),legend.position = "none")
...
anova.stats.meddist <- data.frame(disturbance=c("fire","insect.disease","none"), y=c(.38,.38,.38),letter=c("A","A","A"))
...
#angle model 
angmodmed <- aov(medAngle ~ factor(disturbance), data=median_vectors_all)
par(mfrow=c(2,2))
plot(angmodmed)
summary(angmodmed)
TukeyHSD(angmodmed)

anova.stats.medang <- data.frame(disturbance=c("fire","insect.disease","none"), y=c(1.5,1.5,1.5),letter=c("A","B","AB"))


angle.boxplot<- ggplot(median_vectors_all, aes(x=disturbance, y=medAngle, fill=disturbance))+
  annotate("rect",xmin = -Inf, xmax = Inf, ymin=0.79, ymax = 2.35, fill=rect.colors[2], alpha = 0.5)+
  annotate("rect",xmin = -Inf, xmax = Inf, ymin=-0.79, ymax = 0.79, fill=rect.colors[4], alpha = 0.5)+
  annotate("rect",xmin = -Inf, xmax = Inf, ymin=-2.35, ymax = -0.79, fill=rect.colors[3], alpha = 0.5)+
  annotate("rect",xmin = -Inf, xmax = Inf, ymin=-3.14, ymax = -2.35, fill=rect.colors[1], alpha = 0.5)+
  annotate("rect",xmin = -Inf, xmax = Inf, ymin=2.35, ymax = 3.14, fill=rect.colors[1], alpha = 0.5)+
  geom_text(aes(x="none",y=2.745),nudge_x=0.4,label="hot/dry",size=3.5)+
  geom_text(aes(x="none",y=1.57),nudge_x=0.4,label="hot/wet",size=3.5)+
  geom_text(aes(x="none",y=0),nudge_x=0.4,label="cold/wet",size=3.5)+
  geom_text(aes(x="none",y=-1.57),nudge_x=0.4,label="cold/dry",size=3.5)+
  geom_text(aes(x="none",y=-2.745),nudge_x=0.4,label="hot/dry",size=3.5)+
  geom_boxplot()+
...
arrowcentroidplot<- ggarrange(arrowsplot1,boxplots, nrow=1,ncol=2, widths=c(2,1))

pdf(file="C:/Users/Katie/Google Drive/FIA project/FIA_rproject/figures/arrowsplot.pdf",width=17,height=8)
...
```{r}
#shifts in median
t.test(for_arrows.all %>% filter(!y == 0 & disturbance == "insect.disease") %>% pull(y))
t.test(for_arrows.all %>% filter(!y == 0 & disturbance == "insect.disease") %>% pull(x))

t.test(for_arrows.all %>% filter(!y == 0 & disturbance == "fire") %>% pull(y))
t.test(for_arrows.all %>% filter(!y == 0 & disturbance == "fire") %>% pull(x))

t.test(for_arrows.all %>% filter(!y == 0 & disturbance == "none") %>% pull(y))
t.test(for_arrows.all %>% filter(!y == 0 & disturbance == "none") %>% pull(x))
...
test<- t.test(for_arrows.all %>% filter(!y == 0 & disturbance == disturbance.list[i]) %>% pull(x))
...
for(i in 1:length(disturbance.list)){
test<- t.test(for_arrows.all %>% filter(!y == 0 & disturbance == disturbance.list[i]) %>% pull(y))
...
#pc1 2.5 fire 
ggqqplot(quants_wide %>% filter(disturbance == "fire") %>% pull(pc1diff_2.5))
shapiro.test(quants_wide %>% filter(disturbance == "fire") %>% pull(pc1diff_2.5))

#pc1 2.5 ID 
ggqqplot(quants_wide %>% filter(disturbance == "insect.disease") %>% pull(pc1diff_2.5))
shapiro.test(quants_wide %>% filter(disturbance == "insect.disease") %>% pull(pc1diff_2.5))

#pc1 2.5 none 
ggqqplot(quants_wide %>% filter(disturbance == "none") %>% pull(pc1diff_2.5))
shapiro.test(quants_wide %>% filter(disturbance == "none") %>% pull(pc1diff_2.5))

#pc1 97.5 fire 
ggqqplot(quants_wide %>% filter(disturbance == "fire") %>% pull(pc1diff_97.5))
shapiro.test(quants_wide %>% filter(disturbance == "fire") %>% pull(pc1diff_97.5))

#pc1 97.5 ID 
ggqqplot(quants_wide %>% filter(disturbance == "insect.disease") %>% pull(pc1diff_97.5))
shapiro.test(quants_wide %>% filter(disturbance == "insect.disease") %>% pull(pc1diff_97.5))

#pc1 97.5 none
ggqqplot(quants_wide %>% filter(disturbance == "none") %>% pull(pc1diff_97.5))
shapiro.test(quants_wide %>% filter(disturbance == "none") %>% pull(pc1diff_97.5))


##pc2
#pc2 2.5 fire 
ggqqplot(quants_wide %>% filter(disturbance == "fire") %>% pull(pc2diff_2.5))
shapiro.test(quants_wide %>% filter(disturbance == "fire") %>% pull(pc2diff_2.5))

#pc2 2.5 ID 
ggqqplot(quants_wide %>% filter(disturbance == "insect.disease") %>% pull(pc2diff_2.5))
shapiro.test(quants_wide %>% filter(disturbance == "insect.disease") %>% pull(pc2diff_2.5))

#pc2 2.5 none
ggqqplot(quants_wide %>% filter(disturbance == "none") %>% pull(pc2diff_2.5))
shapiro.test(quants_wide %>% filter(disturbance == "none") %>% pull(pc2diff_2.5))

#pc2 97.5 fire
ggqqplot(quants_wide %>% filter(disturbance == "fire") %>% pull(pc2diff_97.5))
shapiro.test(quants_wide %>% filter(disturbance == "fire") %>% pull(pc2diff_97.5))

#pc2 97.5 ID
ggqqplot(quants_wide %>% filter(disturbance == "insect.disease") %>% pull(pc2diff_97.5))
shapiro.test(quants_wide %>% filter(disturbance == "insect.disease") %>% pull(pc2diff_97.5))

#pc2 97.5 none 
ggqqplot(quants_wide %>% filter(disturbance == "none") %>% pull(pc2diff_97.5))
shapiro.test(quants_wide %>% filter(disturbance == "none") %>% pull(pc2diff_97.5))
...
trait.data <- data.frame(species.name = species.names$species.name,
                         shade.tolerance=c(4.33,4.01,4.83,4.53,1,1.44,1.48,1.56,1.64,1.17,2.78,4.73,1.21,2.09),
                         dispersal = c(rep("wind",4),rep("animal",2),"wind","animal","wind","animal",rep("wind",3),"animal"))
...
traits.plot8 <- ggarrange(shade.exp.plot,shade.contr.plot ,weight.exp.plot,  weight.contr.plot, pc1.exp.plot, pc1.contr.plot, nrow=5, ncol=2, disp.exp.plot,disp.contr.plot,  common.legend = TRUE, labels= c("a","","b","","c","","d"), font.label = list(size=24))

ggarrange(diff.plot,traits.plot8,ncol=1,nrow=2,heights=c(1.5,4))


png("traitplot.png", width = 800, height = 1600)
...
plot.data %>% 
  ...
  dplyr::summarise(n=n()) ##so i do have non-forest plots in here...is this a problem???
plot.data %>% 
...
survey<- read_csv("C:/Users/Katie/Google Drive/FIA project/compiled_data_annual2020/SURVEY.csv")
